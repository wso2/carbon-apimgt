...
class ViewKeys extends React.Component {
  ...
  handleShowToken() {
    ...
    let promised_tokens = this.state.application.generateToken(
      this.props.key_type
    );
  ...
  handleTextChange(event) {
    ...
    let keys = application.keys.get(this.props.key_type) || {
      supportedGrantTypes: ["client_credentials"],
   ...
   */
  componentDidMount() {
    let promised_app = Application.get(this.props.selectedApp.appId);
    promised_app
      .then(application => {
  ...
  render() {
    const { notFound, showCS, showToken, showCurl } = this.state;
    const { keyType, classes, fullScreen, selectedApp } = this.props;
    if (notFound) {
    ...
    let cs_ck_keys = this.state.application.keys.get(keyType);
    ...
    return consumerKey ? (
          ...
          <Grid container spacing={24}>
            ...
            <Grid item xs={6}>
              ...
              <div className={classes.copyWrapper}>
                ...
                <Tooltip
                  title={
                    this.state.secretCopied ? "Copied" : "Copy to clipboard"
                  }
                  placement="right"
                >
                  <CopyToClipboard
      ...
      <React.Fragment>
        <Typography variant="caption" gutterBottom className={classes.noKeyMessageBox}>
          {keyType} Key and Secret is not generated for this application
        </Typography>
      </React.Fragment>
    );
  }
