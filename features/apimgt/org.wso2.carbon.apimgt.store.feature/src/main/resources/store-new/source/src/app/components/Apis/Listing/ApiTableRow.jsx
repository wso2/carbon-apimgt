...
class APiTableRow extends React.Component {
    ...
    handleApiDelete(e) {
        ...
        const promised_delete = api.deleteAPI(api_uuid);
    ...
    componentDidMount() {
        ...
        const promised_api = api.getAPIById(this.props.api.id);
        const existing_subscriptions = api.getSubscriptions(this.props.api.id, null);
        const promised_applications = api.getAllApplications();
        const promised_rating = api.getRatingFromUser(this.props.api.id, null);

        Promise.all([promised_api, existing_subscriptions, promised_applications, promised_rating])
            .then((response) => {
                const [api, subscriptions, applications, rating] = response.map(data => data.obj);
                this.setState({
    ...
    render() {
        const {
            name, version, context, id,
        } = this.props.api;
        const details_link = '/apis/' + id;
        const {
            collapseOpen, subscribedApplications, rating, description, policies,
        } = this.state;
        ...
        const apiRows = [
            <TableRow hover onClick={this.handleClick} key={'row-data-' + id}>
                ...
                <TableCell className={collapseOpen ? classes.root : null}>{version}</TableCell>
                <TableCell className={collapseOpen ? classes.root : null}>{context}</TableCell>
                <TableCell className={collapseOpen ? classes.root : null}>
                ...
                <TableRow key={'row-data-child-' + id}>
                    <TableCell colSpan='4'>
                        ...
                        <Typography variant='body2'>
                            Policies:
                            {policies.length === 0 ? <span>&lt; NOT SET FOR THIS API &gt;</span> : null}
                            {policies.map((policy, index) => (
                                <span key={'span-policy-' + index}>
                                    {' ' + policy}
                                    {policies.length !== 1 && index !== policies.length - 1 && <span>,</span>}
                                </span>
                            ))}
                        </Typography>
                        <Typography variant='body2'>
                            Subscribed Apps:
                            {subscribedApplications.length === 0 ? <span>&lt; NOT SET FOR THIS API &gt;</span> : null}
                            {subscribedApplications.map((app, index) => (
                                <Link to={'/applications/' + app.value} key={'app-link-' + app.value}>
                                    {' ' + app.label}
                                    {subscribedApplications.length !== 1 && index !== subscribedApplications.length - 1 && <span>,</span>}
                                </Link>
                            ))}
                        </Typography>
                    </TableCell>
