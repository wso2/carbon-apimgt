...
APIDesigner.prototype.init_controllers = function(){
    ...
    $("#context").change(function(e){
        ...
        APIDesigner().api_doc.basePath = getContextValue()
        });
    ...
    $("#scope_submit").click(function(){
		...
		jagg.post("/site/blocks/composite-api/composite-api-add/ajax/add.jag",
		    {
...
$(document).ready(function(){
    ...
    $('#import_swagger').click(function () {
    	...
    	if ($('.toggleRadios input[type=radio]:checked').val() == 'swagger_import_file') {
            ...
            $.get(jagg.site.context + "/site/blocks/composite-api/composite-api-add/ajax/import.jag", data,
            function (data) {
                $('#import_swagger').buttonLoader('stop');
                $('#swagger_help').hide();
                $("#swaggerUpload").modal('hide');
