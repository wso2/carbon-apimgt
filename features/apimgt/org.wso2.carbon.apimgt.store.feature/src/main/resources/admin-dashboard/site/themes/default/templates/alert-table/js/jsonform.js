...
jsonform.elementTypes = {
  ...
  'endpoint':{
    'template':
      ...
      '<button class="btn check_url_valid" type="button" providerName="<%= apiProvider%>" apiName="<%= apiName%>" apiVersion="<%= apiVersion %>" >Test</button>'+
  ...
  'multiselect':{
    'template':'<select multiple="multiple" name="<%= node.name %>" id="<%= id %>"' +
      '<%= (fieldHtmlClass ? " class=\'" + fieldHtmlClass + "\'" : "") %>' +
      '<%= (node.disabled? " disabled" : "")%>' +
      '<%= (node.schemaElement && node.schemaElement.required ? " required=\'required\'" : "") %>' +
      '> ' +
      '<% _.each(node.formElement.titleMap, function(key, val) { if(key instanceof Object) { if ($.inArray(String(key.value), value) > -1) { %> <option selected value="<%= val %>"><%= key.title %></option> <% } else { %> <option value="<%= key.value %>"><%= key.title %></option> <% }} else { if ($.inArray(String(val), value) > -1) { %> <option selected value="<%= val %>"><%= key %></option> <% } else { %><option value="<%= val %>"><%= key %></option> <% }}}); %> ' +
      '</select>',
    'fieldtemplate': true,
    'inputfield': true
  },
...
formNode.prototype.getFormValues = function (updateArrayPath) {
  ...
  for (var i = 0; i < formArray.length; i++) {
    ...
    if (eltSchema._jsonform_multiselect_as_array) {
        var mularray = jsonform.util.getObjKey(values, formArray[i].name);
        if(! (mularray instanceof Array))
            mularray = [];
        mularray.push(formArray[i].value);
        jsonform.util.setObjKey(values, formArray[i].name, mularray);
        continue;
    }
...
formTree.prototype.buildFromLayout = function (formElement, context) {
  ...
  if (formElement.key) {
    ...
    if(formElement.type === 'multiselect'){
        schemaElement._jsonform_multiselect_as_array = true;
    }
...
formTree.prototype.getValues = function(){
