...
/
CREATE TABLE AM_BOT_DATA (
    REQUEST_TIME NUMBER(19),
...
CREATE TABLE HONEYPOT_EMAIL_LIST (
    EMAIL_LIST_ID NUMBER(10),
    TENANT_DOMAIN VARCHAR2(255) NOT NULL ,
    EMAIL_LIST VARCHAR2(255),
    PRIMARY KEY (EMAIL_LIST_ID,TENANT_DOMAIN) );

-- Generate ID using sequence and trigger
CREATE SEQUENCE HONEYPOT_EMAIL_LIST_seq START WITH 1 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER HONEYPOT_EMAIL_LIST_seq_tr
     BEFORE INSERT ON HONEYPOT_EMAIL_LIST FOR EACH ROW
     WHEN (NEW.EMAIL_LIST_ID IS NULL)
    BEGIN
     SELECT HONEYPOT_EMAIL_LIST_seq.NEXTVAL INTO :NEW.EMAIL_LIST_ID FROM DUAL;
    END;
/
