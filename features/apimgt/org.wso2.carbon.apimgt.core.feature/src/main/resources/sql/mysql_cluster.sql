...
-- Environment to Data Plane mapping Table --
CREATE TABLE IF NOT EXISTS CHOREO_GW_ENV_DATA_PLANE_MAPPING (
  GATEWAY_ENV_UUID VARCHAR(45) NOT NULL,
  DATA_PLANE_ID VARCHAR(255) NOT NULL,
  FOREIGN KEY (GATEWAY_ENV_UUID) REFERENCES AM_GATEWAY_ENVIRONMENT(UUID) ON UPDATE CASCADE ON DELETE CASCADE,
  PRIMARY KEY (GATEWAY_ENV_UUID, DATA_PLANE_ID)
)ENGINE=NDB;
...
CREATE TABLE IF NOT EXISTS CHOREO_AM_API (
    API_UUID VARCHAR(256),
    API_THROTTLE_LIMIT VARCHAR(256),
    FOREIGN KEY (API_UUID) REFERENCES AM_API(API_UUID) ON DELETE CASCADE,
    PRIMARY KEY(API_UUID)
)ENGINE=NDB;

CREATE TABLE IF NOT EXISTS CHOREO_AM_API_REVISION_METADATA (
  API_UUID VARCHAR(64),
  REVISION_UUID VARCHAR(255),
  API_THROTTLE_LIMIT VARCHAR(256),
...
CREATE TABLE IF NOT EXISTS CHOREO_API_ASGARDEO_MAPPING
(
  CHOREO_API_UUID     VARCHAR(256),
  ASGARDEO_API_UUID      VARCHAR(256),
  PRIMARY KEY (CHOREO_API_UUID)
)ENGINE=NDB;

CREATE TABLE IF NOT EXISTS CHOREO_SUBSCRIPTION_ASGARDEO_MAPPING
(
  CHOREO_SUBSCRIPTION_UUID     VARCHAR(256),
  ASGARDEO_SUBSCRIPTION_UUID      VARCHAR(256),
  PRIMARY KEY (CHOREO_SUBSCRIPTION_UUID)
)ENGINE=NDB;
