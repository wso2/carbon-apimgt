...
CREATE TABLE IF NOT EXISTS AM_GLOBAL_POLICY (
   GLOBAL_POLICY_ID INTEGER AUTO_INCREMENT,
   POLICY_UUID VARCHAR(45) NOT NULL,
   GATEWAY_LABEL VARCHAR(45) NOT NULL,
   IS_DEPLOYED CHAR(1) DEFAULT '0',
   FOREIGN KEY (POLICY_UUID) REFERENCES AM_OPERATION_POLICY(POLICY_UUID) ON DELETE CASCADE,
   FOREIGN KEY (GATEWAY_LABEL) REFERENCES AM_GW_API_DEPLOYMENTS(LABEL) ON DELETE CASCADE,
   PRIMARY KEY(GLOBAL_POLICY_ID)
)ENGINE INNODB;

CREATE TABLE IF NOT EXISTS AM_GLOBAL_POLICY_MAPPING (
   GLOBAL_POLICY_MAPPING_ID INTEGER AUTO_INCREMENT,
   POLICY_UUID VARCHAR(45) NOT NULL,
   POLICY_ORDER INTEGER NOT NULL,
   DIRECTION VARCHAR(10) NOT NULL,
   PARAMETERS VARCHAR(1024) NOT NULL,
   FOREIGN KEY (POLICY_UUID) REFERENCES AM_GLOBAL_POLICY(GLOBAL_POLICY_ID) ON DELETE CASCADE,
   PRIMARY KEY(GLOBAL_POLICY_MAPPING_ID)
)ENGINE INNODB;
