...
CREATE OR REPLACE FUNCTION update_am_api_minhash_updated_time() RETURNS TRIGGER AS $$ BEGIN NEW.UPDATED_TIME = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_am_api_minhash_updated_time ON AM_API_MINHASH;
CREATE TRIGGER trg_am_api_minhash_updated_time BEFORE UPDATE ON AM_API_MINHASH FOR EACH ROW EXECUTE FUNCTION update_am_api_minhash_updated_time();
