...
CREATE TABLE IF NOT EXISTS AM_KM_MGT_APPLICATION (
    ID INTEGER NOT NULL AUTO_INCREMENT,
    CONSUMER_KEY VARCHAR (255),
    CONSUMER_SECRET VARCHAR (2048),
    TENANT_ID INTEGER NOT NULL,
    CONSTRAINT KM_TENANT_CONSUMER_KEY_CONSTRAINT UNIQUE (CONSUMER_KEY, TENANT_ID),
    PRIMARY KEY (ID)
);
...
CREATE TABLE IF NOT EXISTS ENDPOINT_REG (
  ...
  UNIQUE (UUID),
  UNIQUE (REG_NAME, TENANT_ID),
  UNIQUE (REG_TYPE, TENANT_ID),
  UNIQUE (DISPLAY_NAME, TENANT_ID),
  PRIMARY KEY(ID)
);
...
CREATE TABLE IF NOT EXISTS ENDPOINT_REG_ENTRY (
  ...
  PRODUCTION_SERVICE_URL VARCHAR(2048),
  SANDBOX_SERVICE_URL VARCHAR(2048),
  DEFINITION_TYPE VARCHAR(20),
  DEFINITION_URL VARCHAR(2048),
  DESCRIPTION VARCHAR(512),
  SERVICE_TYPE VARCHAR(20),
  SERVICE_CATEGORY VARCHAR(50),
  ENDPOINT_DEFINITION BLOB,
  ...
  UPDATED_BY VARCHAR(255),
  PRIMARY KEY(UUID),
  UNIQUE (ENTRY_NAME, ENTRY_VERSION, REG_ID),
  UNIQUE (DISPLAY_NAME, ENTRY_VERSION, REG_ID),
...
CREATE TABLE IF NOT EXISTS AM_API_ENDPOINT_REG_ENTRY_MAPPING (
    ...
    ENTRY_VERSION VARCHAR(30),
    UNIQUE (API_ID, REG_NAME, ENTRY_NAME, ENTRY_VERSION),
    FOREIGN KEY (API_ID) REFERENCES AM_API(API_ID) ON DELETE CASCADE,
);
