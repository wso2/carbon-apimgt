...
CREATE TABLE AM_API_ENDPOINTS (
 ...
 UNIQUE (ENDPOINT_UUID, REVISION_UUID)
)
/
-- Generate ID using sequence and trigger
CREATE SEQUENCE AM_API_ENDPOINTS_seq START WITH 1 INCREMENT BY 1
/
CREATE OR REPLACE TRIGGER AM_API_ENDPOINTS_seq_tr
 BEFORE INSERT ON AM_API_ENDPOINTS FOR EACH ROW
 WHEN (NEW.ENDPOINT_ID IS NULL)
BEGIN
 SELECT AM_API_ENDPOINTS_seq.NEXTVAL INTO :NEW.ENDPOINT_ID FROM DUAL;
END;
/

CREATE TABLE AM_API_OPERATION_ENDPOINT_MAPPING (
 MAPPING_ID NUMBER(10) NOT NULL,
 URL_MAPPING_ID NUMBER(10) NOT NULL,
 ENDPOINT_ID NUMBER(10) NOT NULL,
 ENVIRONMENT VARCHAR2(100) CHECK( ENVIRONMENT IN ('SANDBOX','PRODUCTION') ),
 FOREIGN KEY (ENDPOINT_ID) REFERENCES AM_API_ENDPOINTS(ENDPOINT_ID) ON DELETE CASCADE,
 FOREIGN KEY (URL_MAPPING_ID) REFERENCES AM_API_URL_MAPPING(URL_MAPPING_ID) ON DELETE CASCADE,
 PRIMARY KEY(MAPPING_ID)
)
/
-- Generate ID using sequence and trigger
CREATE SEQUENCE AM_API_OPERATION_ENDPOINT_MAPPING_seq START WITH 1 INCREMENT BY 1;
/
CREATE OR REPLACE TRIGGER AM_API_OPERATION_ENDPOINT_MAPPING_seq_tr
 BEFORE INSERT ON AM_API_OPERATION_ENDPOINT_MAPPING FOR EACH ROW
 WHEN (NEW.MAPPING_ID IS NULL)
BEGIN
 SELECT AM_API_OPERATION_ENDPOINT_MAPPING_seq.NEXTVAL INTO :NEW.MAPPING_ID FROM DUAL;
END;
/

CREATE TABLE AM_API_PRIMARY_ENDPOINT_MAPPING (
 MAPPING_ID NUMBER(10) NOT NULL,
 API_ID NUMBER(10) NOT NULL,
 ENDPOINT_ID NUMBER(10) NOT NULL,
 ENVIRONMENT VARCHAR2(100) CHECK( ENVIRONMENT IN ('SANDBOX','PRODUCTION') ),
...
/
-- Generate ID using sequence and trigger
CREATE SEQUENCE AM_API_PRIMARY_ENDPOINT_MAPPING_seq START WITH 1 INCREMENT BY 1;
/
CREATE OR REPLACE TRIGGER AM_API_PRIMARY_ENDPOINT_MAPPING_seq_tr
 BEFORE INSERT ON AM_API_PRIMARY_ENDPOINT_MAPPING FOR EACH ROW
 WHEN (NEW.MAPPING_ID IS NULL)
BEGIN
 SELECT AM_API_PRIMARY_ENDPOINT_MAPPING_seq.NEXTVAL INTO :NEW.MAPPING_ID FROM DUAL;
END;
