...
CREATE TABLE AM_API_METADATA (
    API_UUID        VARCHAR(256) NOT NULL,
    REVISION_UUID   VARCHAR(255) DEFAULT 'Current API' NOT NULL,
    METADATA_KEY    VARCHAR(256) NOT NULL,
    METADATA_VALUE  VARCHAR(1024) NOT NULL,
    FOREIGN KEY (API_UUID) REFERENCES AM_API(API_UUID),
    PRIMARY KEY (API_UUID, REVISION_UUID, METADATA_KEY)
)
/

CREATE TABLE AM_API_ENDPOINTS (
    API_UUID VARCHAR2(256) NOT NULL,
    ENDPOINT_UUID VARCHAR2(100) NOT NULL,
    REVISION_UUID VARCHAR2(100) DEFAULT 'Current API' NOT NULL,
    ENDPOINT_NAME VARCHAR2(255) NOT NULL,
    ...
    PRIMARY KEY (API_UUID, ENDPOINT_UUID, REVISION_UUID, ORGANIZATION)
)
/

CREATE TABLE AM_BACKEND (
    BACKEND_ID                  VARCHAR(40) NOT NULL,
    BACKEND_NAME                VARCHAR(256) NOT NULL,
    ENDPOINT_CONFIG             BLOB,
    DEFINITION                  BLOB,
    REFERENCE_API_UUID          VARCHAR(256) NOT NULL,
    REFERENCE_API_REVISION_UUID VARCHAR(256) DEFAULT 'Current API' NOT NULL,
...
CREATE TABLE AM_BACKEND_OPERATION_MAPPING (
    MAPPING_ID       INTEGER,
    ...
    FOREIGN KEY (URL_MAPPING_ID)
        REFERENCES AM_API_URL_MAPPING (URL_MAPPING_ID),
    FOREIGN KEY (BACKEND_ID)
        REFERENCES AM_BACKEND (BACKEND_ID),
    PRIMARY KEY (MAPPING_ID)
)
/

CREATE SEQUENCE AM_BACKEND_OPERATION_MAPPING_SEQUENCE START WITH 1 INCREMENT BY 1
/

CREATE OR REPLACE TRIGGER AM_BACKEND_OPERATION_MAPPING_TRIGGER
		            BEFORE INSERT
                    ON AM_BACKEND_OPERATION_MAPPING
                    REFERENCING NEW AS NEW
                    FOR EACH ROW
                    BEGIN
                    SELECT AM_BACKEND_OPERATION_MAPPING_SEQUENCE.nextval INTO :NEW.MAPPING_ID FROM dual;
                    END;
/
...
CREATE TABLE AM_API_OPERATION_MAPPING (
    MAPPING_ID          INTEGER,
    URL_MAPPING_ID      INTEGER NOT NULL,
    REF_URL_MAPPING_ID  INTEGER NOT NULL,
    FOREIGN KEY (URL_MAPPING_ID)
        REFERENCES AM_API_URL_MAPPING (URL_MAPPING_ID),
...
CREATE SEQUENCE AM_API_OPERATION_MAPPING_SEQUENCE START WITH 1 INCREMENT BY 1
/

CREATE OR REPLACE TRIGGER AM_API_OPERATION_MAPPING_TRIGGER
		            BEFORE INSERT
                    ON AM_API_OPERATION_MAPPING
                    REFERENCING NEW AS NEW
                    FOR EACH ROW
                    BEGIN
                    SELECT AM_API_OPERATION_MAPPING_SEQUENCE.nextval INTO :NEW.MAPPING_ID FROM dual;
                    END;
/
...
/
CREATE TABLE AM_GW_PLATFORM_API_ARTIFACTS (
  API_ID varchar(255) NOT NULL,
  REVISION_ID varchar(255) NOT NULL,
...
CREATE TABLE AM_GATEWAY_ENVIRONMENT (
            ...
            ENV_MODE VARCHAR2(45) DEFAULT 'WRITE_ONLY' NOT NULL,
...
/
-- Platform gateway events (multi-CP WebSocket sync; PAYLOAD = UTF-8 JSON: optional {metadata, message} envelope or legacy raw wire JSON) --
CREATE TABLE AM_GW_PLATFORM_EVENT (
  ...
  CLAIMED_BY VARCHAR2(36) NULL,
  PRIMARY KEY (ID),
  FOREIGN KEY (GATEWAY_ID) REFERENCES AM_GATEWAY_ENVIRONMENT(UUID) ON DELETE CASCADE
)
/
CREATE INDEX AM_GW_PLATFORM_EVENT_PENDING ON AM_GW_PLATFORM_EVENT (GATEWAY_ID, DELIVERED_AT)
/
...
CREATE TABLE AM_LLM_PROVIDER (
    ...
    MODEL_FAMILY_SUPPORTED VARCHAR(5) DEFAULT 'false' NOT NULL,
...
CREATE TABLE GOV_DEVPORTAL_TEMPLATE (
    TEMPLATE_ID VARCHAR2(36) NOT NULL,
    NAME VARCHAR2(256) NOT NULL,
    DESCRIPTION VARCHAR2(1024),
    FORM_CONFIG CLOB NOT NULL,
    FORM_CONFIG_HASH VARCHAR2(64),
    ...
    UPDATED_BY VARCHAR2(256),
    LAST_UPDATED_TIME TIMESTAMP,
    CONSTRAINT UK_GOV_DPT_NAME_ORG UNIQUE (NAME, ORGANIZATION),
    PRIMARY KEY (TEMPLATE_ID)
)
/

CREATE TABLE GOV_TEMPLATE_RULESET_BINDING (
    BINDING_ID VARCHAR2(36) NOT NULL,
    TEMPLATE_ID VARCHAR2(36) NOT NULL,
    RULESET_ID VARCHAR2(36) NOT NULL,
    BINDING_ORDER INTEGER DEFAULT 0 NOT NULL,
    CREATED_BY VARCHAR2(256),
    CREATED_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT UK_GOV_TRB_TMPL_RULE UNIQUE (TEMPLATE_ID, RULESET_ID),
    CONSTRAINT FK_GOV_TRB_TMPL FOREIGN KEY (TEMPLATE_ID) REFERENCES GOV_DEVPORTAL_TEMPLATE(TEMPLATE_ID) ON DELETE CASCADE,
    CONSTRAINT FK_GOV_TRB_RULESET FOREIGN KEY (RULESET_ID) REFERENCES GOV_RULESET(RULESET_ID),
    PRIMARY KEY (BINDING_ID)
)
/

CREATE TABLE GOV_TEMPLATE_RULESET_KM_SCOPE (
    BINDING_ID VARCHAR2(36) NOT NULL,
    KEY_MANAGER_UUID VARCHAR2(50) NOT NULL,
    ORGANIZATION VARCHAR2(128) NOT NULL,
    CONSTRAINT FK_GOV_TRKS_BINDING FOREIGN KEY (BINDING_ID) REFERENCES GOV_TEMPLATE_RULESET_BINDING(BINDING_ID) ON DELETE CASCADE,
    CONSTRAINT FK_GOV_TRKS_KM FOREIGN KEY (KEY_MANAGER_UUID) REFERENCES AM_KEY_MANAGER(UUID) ON DELETE CASCADE,
    PRIMARY KEY (BINDING_ID, KEY_MANAGER_UUID)
)
/

CREATE TABLE GOV_APP_GOVERNANCE_SNAPSHOT (
    SNAPSHOT_ID VARCHAR2(36) NOT NULL,
    APPLICATION_ID INTEGER NOT NULL,
    APPLICATION_UUID VARCHAR2(256) NOT NULL,
    SOURCE_TEMPLATE_ID VARCHAR2(36) NOT NULL,
    TEMPLATE_NAME VARCHAR2(256) NOT NULL,
    TEMPLATE_DESCRIPTION VARCHAR2(1024),
    FORM_CONFIG CLOB NOT NULL,
    FORM_CONFIG_HASH VARCHAR2(64),
    ORGANIZATION VARCHAR2(128) NOT NULL,
    CAPTURED_BY VARCHAR2(256),
    CAPTURED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT UK_GOV_AGS_APP UNIQUE (APPLICATION_ID),
    CONSTRAINT FK_GOV_AGS_APP FOREIGN KEY (APPLICATION_ID) REFERENCES AM_APPLICATION(APPLICATION_ID) ON DELETE CASCADE,
    PRIMARY KEY (SNAPSHOT_ID)
)
/

CREATE TABLE GOV_APP_RULESET_SNAPSHOT (
    SNAPSHOT_RULESET_ID VARCHAR2(36) NOT NULL,
    SNAPSHOT_ID VARCHAR2(36) NOT NULL,
    SOURCE_RULESET_ID VARCHAR2(36) NOT NULL,
    RULESET_NAME VARCHAR2(256) NOT NULL,
    RULESET_DESCRIPTION VARCHAR2(1024),
    ARTIFACT_TYPE VARCHAR2(128) NOT NULL,
    RULESET_TYPE VARCHAR2(128),
    YAML_CONTENT CLOB NOT NULL,
    CONTENT_SHA256 VARCHAR2(64) NOT NULL,
    BINDING_ORDER INTEGER DEFAULT 0 NOT NULL,
    CONSTRAINT UK_GOV_ARS_SNAP_RULE UNIQUE (SNAPSHOT_ID, SOURCE_RULESET_ID),
    CONSTRAINT FK_GOV_ARS_SNAPSHOT FOREIGN KEY (SNAPSHOT_ID) REFERENCES GOV_APP_GOVERNANCE_SNAPSHOT(SNAPSHOT_ID) ON DELETE CASCADE,
    PRIMARY KEY (SNAPSHOT_RULESET_ID)
)
/

CREATE TABLE GOV_APP_RULE_SNAP_KM_SCOPE (
    SNAPSHOT_RULESET_ID VARCHAR2(36) NOT NULL,
    KEY_MANAGER_UUID VARCHAR2(50) NOT NULL,
    KEY_MANAGER_NAME VARCHAR2(256),
    CONSTRAINT FK_GOV_ARS_KM_RULE_SNAP FOREIGN KEY (SNAPSHOT_RULESET_ID) REFERENCES GOV_APP_RULESET_SNAPSHOT(SNAPSHOT_RULESET_ID) ON DELETE CASCADE,
    PRIMARY KEY (SNAPSHOT_RULESET_ID, KEY_MANAGER_UUID)
)
/
...
CREATE TABLE AM_GW_INSTANCES (
    GATEWAY_ID INTEGER,
    GATEWAY_UUID VARCHAR2(255) NOT NULL,
    ORGANIZATION VARCHAR2(128) NOT NULL,
...
CREATE SEQUENCE AM_GW_INSTANCES_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/

CREATE OR REPLACE TRIGGER AM_GW_INSTANCES_TRIG
            BEFORE INSERT
            ON AM_GW_INSTANCES
            REFERENCING NEW AS NEW
            FOR EACH ROW
              BEGIN
                SELECT AM_GW_INSTANCES_SEQ.nextval INTO :NEW.GATEWAY_ID FROM dual;
              END;
/

CREATE TABLE AM_GW_INSTANCE_ENV_MAPPING (
    GATEWAY_ID INTEGER NOT NULL,
    ENV_LABEL VARCHAR2(255) NOT NULL,
    PRIMARY KEY (GATEWAY_ID, ENV_LABEL),
    FOREIGN KEY (GATEWAY_ID) REFERENCES AM_GW_INSTANCES(GATEWAY_ID) ON DELETE CASCADE
)
/

CREATE TABLE AM_GW_REVISION_DEPLOYMENT (
    GATEWAY_ID INTEGER NOT NULL,
    API_ID VARCHAR2(256) NOT NULL,
    ORGANIZATION VARCHAR2(128) NOT NULL,
    STATUS VARCHAR2(255) NOT NULL,
    ACTION VARCHAR2(255) NOT NULL,
    REVISION_UUID VARCHAR2(255),
    LAST_UPDATED NUMBER(19) NOT NULL,
...
CREATE TABLE AM_TASK_LOCK (
    LOCK_TIME NUMBER(19) NOT NULL,
...
CREATE TABLE AM_API_KEY_API_MAPPING (
...
CREATE TABLE AM_API_KEY_APPLICATION_MAPPING (
            API_KEY_UUID VARCHAR(64),
            APPLICATION_UUID VARCHAR(64),
