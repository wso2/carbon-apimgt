...
/
CREATE SEQUENCE SP_DEFAULT_AUTH_SEQ_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/
CREATE OR REPLACE TRIGGER SP_DEFAULT_AUTH_SEQ_TRIG
  BEFORE INSERT
  ON SP_DEFAULT_AUTH_SEQ
  REFERENCING NEW AS NEW
  FOR EACH ROW
  BEGIN
    SELECT SP_DEFAULT_AUTH_SEQ_SEQ.nextval
    INTO :NEW.ID
    FROM dual;
  END;
/
...
/
CREATE SEQUENCE IDN_TEMPLATE_MGT_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/
CREATE OR REPLACE TRIGGER IDN_TEMPLATE_MGT_TRIG
BEFORE INSERT
ON IDN_TEMPLATE_MGT
REFERENCING NEW AS NEW
FOR EACH ROW
  BEGIN
    SELECT IDN_TEMPLATE_MGT_SEQ.nextval
...
CREATE TABLE IF NOT EXISTS AM_SECURITY_AUDIT_UUID_MAPPING (
    API_ID INTEGER NOT NULL,
    AUDIT_UUID VARCHAR(256) NOT NULL,
    PRIMARY KEY (API_ID, AUDIT_UUID),
...
CREATE TABLE IF NOT EXISTS AM_API_PRODUCT (
	...
	API_PRODUCT_PROVIDER VARCHAR(200),
	API_PRODUCT_NAME VARCHAR(200),
	API_PRODUCT_VERSION VARCHAR(30),
	API_PRODUCT_CONTEXT VARCHAR(30),
	API_PRODUCT_TIER VARCHAR(256),
	CREATED_BY VARCHAR(100),
	CREATED_TIME TIMESTAMP,
	UPDATED_BY VARCHAR(100),
	UPDATED_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
	PRIMARY KEY(API_PRODUCT_ID),
	UNIQUE (API_PRODUCT_PROVIDER,API_PRODUCT_NAME,API_PRODUCT_VERSION)
)
/

CREATE SEQUENCE AM_API_PRODUCT_SEQUENCE START WITH 1 INCREMENT BY 1
/

CREATE OR REPLACE TRIGGER AM_API_PRODUCT_TRIGGER
		            BEFORE INSERT
                    ON AM_API_PRODUCT
                    REFERENCING NEW AS NEW
                    FOR EACH ROW
                    BEGIN
                    SELECT AM_API_PRODUCT_SEQUENCE.nextval INTO :NEW.API_PRODUCT_ID FROM dual;
                    END;
/
...
CREATE TABLE IF NOT EXISTS AM_API_PRODUCT_MAPPING (
	...
	API_PRODUCT_ID INTEGER,
	URL_MAPPING_ID INTEGER,
	FOREIGN KEY (API_PRODUCT_ID) REFERENCES AM_API_PRODUCT(API_PRODUCT_ID) ON DELETE CASCADE,
...
CREATE OR REPLACE TRIGGER AM_API_PRODUCT_MAPPING_TRIGGER
                    ...
                    FOR EACH ROW
                    BEGIN
                    SELECT AM_API_PRODUCT_MAPPING_SEQUENCE.nextval INTO :NEW.API_PRODUCT_MAPPING_ID FROM dual;
                    END;
/
