...
CREATE TABLE IF NOT EXISTS AM_OPERATION_POLICY_TEMPLATES (
    TEMPLATE_ID INTEGER AUTO_INCREMENT,
    TEMPLATE_NAME VARCHAR(300) NOT NULL,
    DISPLAY_NAME VARCHAR(300),
    POLICY_DESCRIPTION VARCHAR(1024),
    FLOW VARCHAR(30) NOT NULL,
    GATEWAY_TYPES VARCHAR(30) NOT NULL,
    API_TYPES VARCHAR(30) NOT NULL,
...
CREATE TABLE IF NOT EXISTS AM_API_OPERATION_POLICY_DEFINITIONS (
    POLICY_ID INTEGER AUTO_INCREMENT,
    TEMPLATE_ID INTEGER,
    API_UUID VARCHAR(300) NOT NULL,
    REVISION_UUID VARCHAR(300),
    ...
    POLICY_PARAMETERS blob,
    POLICY_DEFINITION blob,
    FOREIGN KEY (TEMPLATE_ID) REFERENCES AM_OPERATION_POLICY_TEMPLATES(TEMPLATE_ID) ON DELETE CASCADE,
    PRIMARY KEY(POLICY_ID)
)
/
...
CREATE TABLE IF NOT EXISTS AM_API_OPERATION_POLICY_MAPPING (
   ...
   FOREIGN KEY (URL_MAPPING_ID) REFERENCES AM_API_URL_MAPPING(URL_MAPPING_ID) ON DELETE CASCADE,
   FOREIGN KEY (POLICY_ID) REFERENCES AM_API_OPERATION_POLICY_DEFINITIONS(POLICY_ID) ON DELETE CASCADE,
   PRIMARY KEY(OPERATION_POLICY_MAPPING_ID),
   UNIQUE (URL_MAPPING_ID, POLICY_ORDER)
)
/

CREATE SEQUENCE AM_API_OPERATION_POLICY_MAPPING_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/

CREATE TABLE IF NOT EXISTS AM_API_RESOURCE_ENDPOINTS (
   RESOURCE_ENDPOINT_ID INTEGER,
   API_UUID VARCHAR(256),
   ...
   SECURITY_CONFIG blob,
   ENDPOINT_CONFIG blob,
   FOREIGN KEY (API_UUID) REFERENCES AM_API(API_UUID) ON DELETE CASCADE,
   PRIMARY KEY(RESOURCE_ENDPOINT_ID),
   UNIQUE (API_UUID, UUID, REVISION_UUID)
)
/

CREATE SEQUENCE AM_API_RESOURCE_ENDPOINTS_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/

CREATE TABLE IF NOT EXISTS AM_API_RESOURCE_ENDPOINT_MAPPING (
   OPERATION_ENDPOINT_MAPPING_ID INTEGER,
   OPERATION_POLICY_MAPPING_ID INTEGER NOT NULL,
   RESOURCE_ENDPOINT_ID INTEGER NOT NULL,
   FOREIGN KEY (OPERATION_POLICY_MAPPING_ID) REFERENCES AM_API_OPERATION_POLICY_MAPPING(OPERATION_POLICY_MAPPING_ID)
         ON DELETE CASCADE,
   FOREIGN KEY (RESOURCE_ENDPOINT_ID) REFERENCES AM_API_RESOURCE_ENDPOINTS(RESOURCE_ENDPOINT_ID) ON DELETE CASCADE,
   PRIMARY KEY(OPERATION_ENDPOINT_MAPPING_ID)
)
/

CREATE SEQUENCE AM_API_RESOURCE_ENDPOINT_MAPPING_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/
