...
IF NOT  EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[AM_GRAPHQL_QUERY_ANALYSIS]') AND TYPE IN (N'U'))

CREATE TABLE AM_GRAPHQL_QUERY_ANALYSIS (
    API_ID INTEGER NOT NULL,
    DEPTH_ENABLED BIT DEFAULT 0,
    COMPLEXITY_ENABLED BIT DEFAULT 0,
    MAX_COMPLEXITY INTEGER,
    FOREIGN KEY (API_ID) REFERENCES AM_API(API_ID) ON UPDATE CASCADE ON DELETE RESTRICT,
    PRIMARY KEY(API_ID)
);

IF NOT  EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[AM_GRAPHQL_DEPTH]') AND TYPE IN (N'U'))
