...
END
GO
IF NOT EXISTS (SELECT * FROM sys.triggers WHERE name = 'TRG_AM_API_MINHASH_UPD')
BEGIN
    EXEC('CREATE TRIGGER TRG_AM_API_MINHASH_UPD ON AM_API_MINHASH AFTER UPDATE AS BEGIN UPDATE AM_API_MINHASH SET UPDATED_TIME = GETDATE() FROM AM_API_MINHASH INNER JOIN inserted ON AM_API_MINHASH.API_UUID = inserted.API_UUID AND AM_API_MINHASH.ORGANIZATION = inserted.ORGANIZATION; END');
END
GO
