...
/
CREATE SEQUENCE SP_DEFAULT_AUTH_SEQ_SEQUENCE START WITH 1 INCREMENT BY 1 NOCACHE
/
CREATE TRIGGER SP_DEFAULT_AUTH_SEQ_SEQUENCE_TRIGGER NO CASCADE
            BEFORE INSERT
            ON SP_DEFAULT_AUTH_SEQ
            REFERENCING NEW AS NEW
            FOR EACH ROW MODE DB2SQL
                BEGIN ATOMIC
                    SET (NEW.ID) = (NEXTVAL FOR SP_DEFAULT_AUTH_SEQ_SEQUENCE);
                END
/
...
/
CREATE SEQUENCE IDN_TEMPLATE_MGT_SEQUENCE START WITH 1 INCREMENT BY 1 NOCACHE
/

CREATE TRIGGER IDN_TEMPLATE_MGT_SEQUENCE_TRIGGER NO CASCADE
    BEFORE INSERT
    ON IDN_TEMPLATE_MGT
    REFERENCING NEW AS NEW
    FOR EACH ROW MODE DB2SQL
      BEGIN ATOMIC
        SET (NEW.TEMPLATE_ID) = (NEXTVAL FOR IDN_TEMPLATE_MGT_SEQUENCE);
      END
/
...
CREATE TABLE AM_SECURITY_AUDIT_UUID_MAPPING (
    API_ID INTEGER NOT NULL GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1),
    AUDIT_UUID VARCHAR(256) NOT NULL,
    PRIMARY KEY (API_ID, AUDIT_UUID),
    FOREIGN KEY (API_ID) REFERENCES AM_API(API_ID)
)/

CREATE TABLE AM_API_PRODUCT (
	API_PRODUCT_ID INTEGER GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1),
