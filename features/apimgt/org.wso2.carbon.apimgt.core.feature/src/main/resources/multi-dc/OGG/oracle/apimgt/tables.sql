...
INSERT INTO CM_PURPOSE (NAME, DESCRIPTION, PURPOSE_GROUP, GROUP_TYPE, TENANT_ID) VALUES
  ...
  BEFORE DELETE
    ...
    FOR EACH ROW
      BEGIN
        ...
        DELETE FROM AM_GW_API_DEPLOYMENTS AGA WHERE AGA.API_ID = :OLD.API_ID;
      END;
/
  ...
  BEFORE DELETE
    ...
    FOR EACH ROW
      BEGIN
        ...
        DELETE FROM IDN_CORS_ASSOCIATION IDS WHERE IDS.SP_APP_ID = :OLD.ID;
      END;
/
  ...
  BEFORE DELETE
    ...
    FOR EACH ROW
      BEGIN
        ...
        DELETE FROM IDN_OAUTH2_CIBA_REQUEST_SCOPES IOCRS WHERE IOCRS.AUTH_CODE_KEY = :OLD.AUTH_CODE_KEY;
      END;
/
  ...
  BEFORE DELETE
    ...
    FOR EACH ROW
      BEGIN
        ...
        DELETE FROM IDN_UMA_RESOURCE_META_DATA IURMD WHERE IURMD.RESOURCE_IDENTITY = :OLD.ID;
        DELETE FROM IDN_UMA_RESOURCE_SCOPE IURS WHERE IURS.RESOURCE_IDENTITY = :OLD.ID;
      END;
/
  ...
  BEFORE DELETE
    ...
    FOR EACH ROW
      BEGIN
        ...
        DELETE FROM AM_REVISION AR WHERE AR.API_UUID = :OLD.API_UUID;
  ...
  BEFORE DELETE
    ...
    FOR EACH ROW
      BEGIN
        ...
        DELETE FROM AM_API_OPERATION_POLICY_MAPPING AAOPM WHERE AARSM.URL_MAPPING_ID = :OLD.URL_MAPPING_ID;
      END;
/
...
CREATE OR REPLACE TRIGGER TRG_DEL_AM_LABELS
  BEFORE DELETE
    ...
    FOR EACH ROW
      ...
      END;
/
CREATE OR REPLACE TRIGGER TRG_DEL_WF_REQ
  BEFORE DELETE
    on WF_REQUEST
    FOR EACH ROW
        BEGIN
            DELETE FROM WF_REQUEST_ENTITY_RELATIONSHIP WRER WHERE WRER.REQUEST_ID = :OLD.UUID;
            DELETE FROM WF_WORKFLOW_REQUEST_RELATION WWRR WHERE WWRR.REQUEST_ID = :OLD.UUID;
        END;
/

CREATE OR REPLACE TRIGGER TRG_DEL_IDN_CFG_TYPE
  BEFORE DELETE
on IDN_CONFIG_TYPE
    FOR EACH ROW
BEGIN
DELETE FROM IDN_CONFIG_RESOURCE ICR WHERE ICR.TYPE_ID = :OLD.ID;
END;
/

CREATE OR REPLACE TRIGGER TRG_DEL_IDN_RMT_FETCH_CFG
  BEFORE DELETE
on IDN_REMOTE_FETCH_CONFIG
    FOR EACH ROW
BEGIN
DELETE FROM IDN_REMOTE_FETCH_REVISIONS IRFR WHERE IRFR.CONFIG_ID = :OLD.ID;
END;
/

CREATE OR REPLACE TRIGGER TRG_DEL_IDN_CORS_ORIGIN
  BEFORE DELETE
on IDN_CORS_ORIGIN
    FOR EACH ROW
BEGIN
DELETE FROM IDN_CORS_ASSOCIATION ICA WHERE ICA.IDN_CORS_ORIGIN_ID = :OLD.ID;
END;
/
...
on AM_SCOPE
    FOR EACH ROW
BEGIN
DELETE FROM AM_SCOPE_BINDING ASB WHERE ASB.SCOPE_ID = :OLD.SCOPE_ID;
END;
/
...
on AM_REVISION
    FOR EACH ROW
BEGIN
DELETE FROM AM_API_REVISION_METADATA AARM WHERE AARM.REVISION_UUID = :OLD.REVISION_UUID;
DELETE FROM AM_DEPLOYMENT_REVISION_MAPPING ADRM WHERE ADRM.REVISION_UUID = :OLD.REVISION_UUID;
DELETE FROM AM_DEPLOYED_REVISION ADR WHERE ADR.REVISION_UUID = :OLD.REVISION_UUID;
END;
/

CREATE OR REPLACE TRIGGER TRG_DEL_AM_GATEWAY_ENV
  BEFORE DELETE
on AM_GATEWAY_ENVIRONMENT
    FOR EACH ROW
BEGIN
DELETE FROM AM_GW_VHOST AGV WHERE AGV.GATEWAY_ENV_ID = :OLD.ID;
END;
/

CREATE OR REPLACE TRIGGER TRG_DEL_AM_OPR_POLICY
  BEFORE DELETE
on AM_OPERATION_POLICY
    FOR EACH ROW
BEGIN
DELETE FROM AM_OPERATION_POLICY_DEFINITION AOPD WHERE AOPD.POLICY_UUID = :OLD.POLICY_UUID;
DELETE FROM AM_COMMON_OPERATION_POLICY ACOP WHERE ACOP.POLICY_UUID = :OLD.POLICY_UUID;
DELETE FROM AM_API_OPERATION_POLICY AAOP WHERE AAOP.POLICY_UUID = :OLD.POLICY_UUID;
DELETE FROM AM_API_OPERATION_POLICY_MAPPING AAOPM WHERE AAOPM.POLICY_UUID = :OLD.POLICY_UUID;
END;
/
