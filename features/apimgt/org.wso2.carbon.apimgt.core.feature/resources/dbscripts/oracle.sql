...
CREATE TABLE AM_ENDPOINT (
  ...
  TPS_SANDBOX NUMBER(10),
  TPS_PRODUCTION NUMBER(10),
...
execute ctx_ddl.create_preference('API_MULTI_COLUMN', 'MULTI_COLUMN_DATASTORE')
/
execute ctx_ddl.set_attribute('API_MULTI_COLUMN', 'columns', 'AM_API.NAME, AM_API.VERSION, AM_API.PROVIDER, AM_API.DESCRIPTION, AM_API.CONTEXT, AM_API.TECHNICAL_OWNER, AM_API.BUSINESS_OWNER, AM_API.CURRENT_LC_STATUS')
/

ALTER TABLE AM_API ADD INDEXER VARCHAR(1)
/

CREATE INDEX API_INDEX ON AM_API(INDEXER) INDEXTYPE IS CTXSYS.CONTEXT PARAMETERS('DATASTORE API_MULTI_COLUMN SYNC (ON COMMIT)')
/
...
CREATE TABLE AM_APPLICATION (
  ...
  GROUP_ID VARCHAR2(255) DEFAULT NULL NULL,
  CREATED_BY VARCHAR2(100),
  CREATED_TIME TIMESTAMP(6) DEFAULT CURRENT_TIMESTAMP(6),
...
CREATE TABLE AM_LAMBDA_FUNCTION (
   ...
   FUNCTION_URL VARCHAR2(255) NOT NULL,
...
CREATE TABLE AM_EVENT_FUNCTION_MAPPING (
   ...
   FOREIGN KEY (EVENT) REFERENCES AM_EVENT(EVENT)
)
/
