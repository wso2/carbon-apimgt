...
CREATE TABLE `AM_RESOURCE_CATEGORIES` (
  `RESOURCE_CATEGORY_ID` VARCHAR(255),
  `DATA_TYPE_ID` VARCHAR(255),
  `RESOURCE_CATEGORY` VARCHAR(255),
  PRIMARY KEY (`RESOURCE_CATEGORY_ID`),
  UNIQUE (`RESOURCE_CATEGORY`),
  FOREIGN KEY (`DATA_TYPE_ID`) REFERENCES `AM_RESOURCE_DATA_TYPES`(`DATA_TYPE_ID`)
);
...
CREATE TABLE `AM_API_RESOURCES_VISIBLE_ROLES` (
  `RESOURCE_ID` VARCHAR(255),
  `ROLE` VARCHAR(255),
  PRIMARY KEY (`RESOURCE_ID`, `ROLE`),
  FOREIGN KEY (`RESOURCE_ID`) REFERENCES `AM_API_RESOURCES`(`RESOURCE_ID`) ON UPDATE CASCADE ON DELETE CASCADE
);
