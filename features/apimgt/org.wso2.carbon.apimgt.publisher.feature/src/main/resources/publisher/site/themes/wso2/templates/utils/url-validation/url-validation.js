$(document).ready(function(){
    $( "body" ).delegate( "button.check_url_valid", "click", function() {
        ...
        if (url == '') {
        	$(btn).parent().parent().after(' <span class="label label-danger url_validate_label"><i class="glyphicon glyphicon-exclamation-sign icon-white" title="missing Url"></i>'+ i18n.t('validationMsgs.missingUrl')+'</span>');
                  ...
                  function (result) {
                      if (!result.error) {
                          if (result.response.response == "success") {
                              $(btn).parent().parent().after(' <span class="label label-success url_validate_label"><i class="glyphicon glyphicon-ok icon-white" title="valid url"></i>' + i18n.t('validationMsgs.valid') + '</span>');

                          } else {
                              if (result.response.isConnectionError) {
                                if (result.response.response == null) {
                                    $(btn).parent().parent().after(' <span class="label label-danger url_validate_label"><i class="glyphicon glyphicon-remove icon-white" title="error-in-correction"></i>' + i18n.t('validationMsgs.invalid') + i18n.t('validationMsgs.errorInConnection') + '</span>');
                                } else { //When an exception is thrown from jsFunction_isURLValid
                                    $(btn).parent().parent().after(' <span class="label label-danger url_validate_label"><i class="glyphicon glyphicon-remove icon-white" title="invalid url"></i>' + i18n.t('validationMsgs.invalid') + '</span>');
                                }
                              } else {
