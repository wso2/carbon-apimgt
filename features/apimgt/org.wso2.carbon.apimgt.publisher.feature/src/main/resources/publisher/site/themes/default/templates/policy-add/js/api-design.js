...
PolicyDesigner.prototype.check_if_resource_exist = function(path, method){
...
PolicyDesigner.prototype.set_default_management_values = function(){
...
PolicyDesigner.prototype.add_default_resource = function(){
    $("#resource_url_pattern").val("*");
...
PolicyDesigner.prototype.get_scopes = function() {
...
PolicyDesigner.prototype.has_resources = function(){
...
PolicyDesigner.prototype.display_elements = function(value, source){
...
PolicyDesigner.prototype.update_elements = function(resource, newValue){
    var swaggerSchema = JSON.parse('{"type":"object"}');
    var API_DESIGNER = PolicyDesigner();
...
PolicyDesigner.prototype.update_elements_boolean = function(resource, newValue){
    if(newValue == "true") 
        newValue = true;
    else
        newValue = false;
...
PolicyDesigner.prototype.init_controllers = function(){
    var API_DESIGNER = this;

    $("#version").change(function(e){
        PolicyDesigner().api_doc.info.version = $(this).val();
        // We do not need the version anymore. With the new plugable version strategy the context will have the version
        PolicyDesigner().baseURLValue = "http://localhost:8280/"+$("#context").val().replace("/","")});
    $("#context").change(function(e){ PolicyDesigner().baseURLValue = "http://localhost:8280/"+$(this).val().replace("/","")});
    $("#name").change(function(e){ PolicyDesigner().api_doc.info.title = $(this).val() });
    $("#description").change(function(e){ PolicyDesigner().api_doc.info.description = $(this).val() });
...
PolicyDesigner.prototype.load_api_document = function(api_document){
...
PolicyDesigner.prototype.render_scopes = function(){
    if($('#scopes-template').length){    
        context = {
...
PolicyDesigner.prototype.transform = function(api_doc){
...
PolicyDesigner.prototype.render_resources = function(){
    context = {
...
PolicyDesigner.prototype.render_resource = function(container){
...
PolicyDesigner.prototype.query = function(path){
    return jsonPath(this.api_doc,path);
}

PolicyDesigner.prototype.add_resource = function(resource, path){
...
PolicyDesigner.prototype.edit_swagger = function(){
...
PolicyDesigner.prototype.close_swagger_editor = function(){
    $("body").removeClass("modal-open");
    $("#se-iframe").remove();
    $("#swaggerEditer").fadeOut("fast");
};

PolicyDesigner.prototype.update_swagger = function(){
    $("body").removeClass("modal-open");
    $("#se-iframe").remove();
    $("#swaggerEditer").fadeOut("fast");    
    var designer =  PolicyDesigner();
...
$(document).ready(function(){
    ...
    var designer = new PolicyDesigner();
