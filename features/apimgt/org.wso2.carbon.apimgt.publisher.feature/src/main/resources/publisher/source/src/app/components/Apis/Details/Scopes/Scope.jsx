...
class Scope extends React.Component {
  ...
  toggleScopeData() {
    const api = new Api();
    let promised_scopes_object = api.getScopeDetail(
      this.props.api_uuid,
      this.props.name
    );
  ...
  removeScope() {
    ...
    const api = new Api();
    let promised_scope_delete = api.deleteScope(
      this.props.api_uuid,
      scope_name
    );
    promised_scope_delete.then(response => {
      if (response.status !== 200) {
        ...
        message.error(
          "Something went wrong while deleting the " + scope_name + " Scope!"
        );
        hideMessage();
        return;
      }
  ...
  handleSubmit() {
    ...
    const api = new Api();
    let promised_scope_update = api.updateScope(
      this.props.api_uuid,
      scope_name,
      this.state.apiScope
    );
    promised_scope_update.then(response => {
      if (response.status !== 200) {
        console.log(response);
        message.error(
          "Something went wrong while updating the " + scope_name + " Scope!"
        );
        hideMessage();
        return;
      }
