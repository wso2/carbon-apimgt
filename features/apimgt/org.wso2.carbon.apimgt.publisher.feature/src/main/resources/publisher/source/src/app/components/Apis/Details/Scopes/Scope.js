...
class Scope extends React.Component{
    ...
    toggleScopeData(){
        const api = new Api();
        let promised_scopes_object = api.getScopeDetail(this.props.api_uuid,this.props.name);
    ...
    removeScope() {
        let scope_name = this.props.name;
        const hideMessage = message.loading("Deleting the Scope ...", 0);
        const api = new Api();
        let promised_scope_delete = api.deleteScope(this.props.api_uuid, scope_name);
        promised_scope_delete.then(
            response => {
    ...
    handleSubmit(){
        ...
        const api = new Api();
        let promised_scope_update = api.updateScope(this.props.api_uuid, scope_name,this.state.apiScope);
        promised_scope_update.then(
            response => {
                if (response.status !== 200) {
                    console.log(response);
        ...
        return (
            <div id={this.props.name}>
                <Row type="flex" justify="start" className="resource-head">
                    ...
                    <Col span={8} style={{textAlign:"right", cursor:"pointer"}} onClick={this.removeScope}><Icon type="delete" /> </Col>
                </Row>
                {this.state.visible ? <div>
                    {this.state.apiScope ?
                        <Row type="flex" justify="start" className="resource-body">
                                <Col span={20}>
                                <Input name="descriptions" placeholder="descriptions" type="text" onChange={this.handleChangeDescription} defaultValue={this.state.apiScope.description}/>
                                </Col>
                                    <Col span={20}>
                                    <TagsInput value={this.state.apiScope.bindings.values} onChange={this.handleChange} onlyUnique={true} inputProps={{ placeholder: 'add a valid role' }}/>
                                    </Col>
                                    <Col span={20}>
                                    <Button loading={this.state.creating} type="primary" onClick={this.handleSubmit}>Update</Button>
                                    </Col>
                        </Row> : <Loading/> }
                    </div> : null }
            </div>
