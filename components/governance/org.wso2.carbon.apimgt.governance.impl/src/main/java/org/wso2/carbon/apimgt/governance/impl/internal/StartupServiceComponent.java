...
public class StartupServiceComponent {
    ...
    @Reference(name = "governance.config.service",
            service = org.wso2.carbon.apimgt.governance.impl.config.GovernanceConfigurationService.class,
            cardinality = ReferenceCardinality.MANDATORY, policy = ReferencePolicy.DYNAMIC,
            unbind = "unsetGovernanceConfigurationService")
    protected void setGovernanceConfigurationService(GovernanceConfigurationService govConfigService) {
        ServiceReferenceHolder.getInstance().setGovernanceConfigurationService(govConfigService);
    }

    protected void unsetGovernanceConfigurationService(GovernanceConfigurationService govConfigService) {
        ServiceReferenceHolder.getInstance().setGovernanceConfigurationService(null);
    }
}
