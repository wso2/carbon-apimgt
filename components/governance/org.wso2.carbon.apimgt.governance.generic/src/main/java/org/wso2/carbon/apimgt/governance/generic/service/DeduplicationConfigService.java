...
public class DeduplicationConfigService {
    ...
    private DeduplicationConfig loadConfigFromDefaultJson() {
        ...
        try {
            APIManagerConfiguration apiManagerConfig = GenericServiceReferenceHolder.getInstance()
        ...
        // Safe hardcoded defaults (matched to GenericConstants)
    ...
    private DeduplicationConfig fetchConfigFromDatabase(String organization) {
        ...
        log.warn("No deduplication ruleset found in database for organization: " + org 
                + ". Using default threshold: " + GenericConstants.DEFAULT_SIMILARITY_THRESHOLD);
        return null;
    }
     ...
     * Supports both root-level keys (like GenericValidationEngine) and nested 'deduplication' section.
     *
    ...
    private DeduplicationConfig parseConfig(String yamlContent) {
        try {
            ...
            // First try root-level keys (as used by GenericValidationEngine)
            if (content.containsKey(GenericConstants.RulesetConfig.SIMILARITY_THRESHOLD)) {
                config.setSimilarityThreshold(
                        parseDouble(content.get(GenericConstants.RulesetConfig.SIMILARITY_THRESHOLD)));
            }

            if (content.containsKey(GenericConstants.RulesetConfig.ENABLED)) {
                config.setEnabled(Boolean.TRUE.equals(
                        content.get(GenericConstants.RulesetConfig.ENABLED)));
            }

            if (content.containsKey(GenericConstants.RulesetConfig.MODE)) {
                config.setMode(String.valueOf(content.get(GenericConstants.RulesetConfig.MODE)));
            }

            if (content.containsKey(GenericConstants.RulesetConfig.NUM_HASH_FUNCTIONS)) {
                config.setNumHashFunctions(
                        Integer.parseInt(String.valueOf(
                                content.get(GenericConstants.RulesetConfig.NUM_HASH_FUNCTIONS))));
            }

            if (content.containsKey(GenericConstants.RulesetConfig.NUM_BANDS)) {
                config.setNumBands(
                        Integer.parseInt(String.valueOf(
                                content.get(GenericConstants.RulesetConfig.NUM_BANDS))));
            }
    ...
    private void parseNestedConfig(Map<String, Object> dedupConfig, DeduplicationConfig config) {
        if (dedupConfig.containsKey(GenericConstants.RulesetConfig.SIMILARITY_THRESHOLD)) {
            config.setSimilarityThreshold(
                    parseDouble(dedupConfig.get(GenericConstants.RulesetConfig.SIMILARITY_THRESHOLD)));
        }

        if (dedupConfig.containsKey(GenericConstants.RulesetConfig.ENABLED)) {
            config.setEnabled(Boolean.TRUE.equals(
                    dedupConfig.get(GenericConstants.RulesetConfig.ENABLED)));
        }

        if (dedupConfig.containsKey(GenericConstants.RulesetConfig.MODE)) {
            config.setMode(String.valueOf(dedupConfig.get(GenericConstants.RulesetConfig.MODE)));
        }

        if (dedupConfig.containsKey(GenericConstants.RulesetConfig.NUM_HASH_FUNCTIONS)) {
            config.setNumHashFunctions(
                    Integer.parseInt(String.valueOf(
                            dedupConfig.get(GenericConstants.RulesetConfig.NUM_HASH_FUNCTIONS))));
        }

        if (dedupConfig.containsKey(GenericConstants.RulesetConfig.NUM_BANDS)) {
            config.setNumBands(
                    Integer.parseInt(String.valueOf(
                            dedupConfig.get(GenericConstants.RulesetConfig.NUM_BANDS))));
        }
    ...
    public static class DeduplicationConfig {
        ...
        private double similarityThreshold = GenericConstants.DEFAULT_SIMILARITY_THRESHOLD;
        private double highConfidenceThreshold = 0.99;
        private String mode = "audit";
        private int numHashFunctions = GenericConstants.DEFAULT_NUM_HASH_FUNCTIONS;
        private int numBands = GenericConstants.DEFAULT_NUM_BANDS;
        private int shingleSize = GenericConstants.NGRAM_SIZE;
        ...
        public void setSimilarityThreshold(double similarityThreshold) {
            ...
            if (similarityThreshold >= GenericConstants.MIN_SIMILARITY_THRESHOLD &&
                    similarityThreshold <= GenericConstants.MAX_SIMILARITY_THRESHOLD) {
