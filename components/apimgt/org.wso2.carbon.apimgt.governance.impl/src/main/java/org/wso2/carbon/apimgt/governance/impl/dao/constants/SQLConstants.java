...
public class SQLConstants {
    ...
    public static final String DELETE_VIOLATIONS_BY_REFERENCED_API_UUID =
            "DELETE FROM GOV_RULE_VIOLATION " +
            "WHERE (MESSAGE LIKE ? OR VIOLATED_PATH LIKE ?) " +
            "AND RULESET_RUN_ID IN ( " +
            "    SELECT GRR.RULESET_RUN_ID FROM GOV_RULESET_RUN GRR " +
            "    JOIN GOV_ARTIFACT GA ON GRR.ARTIFACT_KEY = GA.ARTIFACT_KEY " +
            "    WHERE GA.ORGANIZATION = ? " +
            ")";

    public static final String GET_AFFECTED_ARTIFACTS_BY_REFERENCED_API_UUID =
            "SELECT DISTINCT GA.ARTIFACT_REF_ID " +
            "FROM GOV_RULE_VIOLATION GRV " +
            "JOIN GOV_RULESET_RUN GRR ON GRV.RULESET_RUN_ID = GRR.RULESET_RUN_ID " +
            "JOIN GOV_ARTIFACT GA ON GRR.ARTIFACT_KEY = GA.ARTIFACT_KEY " +
            "WHERE (GRV.MESSAGE LIKE ? OR GRV.VIOLATED_PATH LIKE ?) " +
            "AND GA.ORGANIZATION = ?";
