...
public class SQLConstants {
    ...
    public static final String DELETE_POLICY_RUN_FOR_ARTIFACT_AND_POLICY = "DELETE GPR " +
            "FROM GOV_POLICY_RUN GPR " +
    ...
    public static final String DELETE_POLICY_RUNS_FOR_ARTIFACT = "DELETE GPR " +
            "FROM GOV_POLICY_RUN GPR " +
            "JOIN GOV_ARTIFACT GA ON GPR.ARTIFACT_KEY = GA.ARTIFACT_KEY " +
            "WHERE GA.ARTIFACT_ID = ? AND GA.ARTIFACT_TYPE = ? AND GA.ORGANIZATION = ?";

    public static final String DELETE_RULESET_RUN_FOR_ARTIFACT_AND_RULESET = "DELETE GRR " +
    ...
    public static final String GET_FAILED_RULESET_RUNS_FOR_ARTIFACT = "SELECT DISTINCT RULESET_ID " +
            "FROM GOV_RULESET_RUN GRR " +
            "JOIN GOV_ARTIFACT GA ON GRR.ARTIFACT_KEY = GA.ARTIFACT_KEY " +
            "WHERE GA.ARTIFACT_ID = ? AND GA.ARTIFACT_TYPE = ? AND GA.ORGANIZATION = ? AND GRR.RESULT = 0";
    public static final String DELETE_RULESET_RUNS_FOR_ARTIFACT = "DELETE GRR " +
            "FROM GOV_RULESET_RUN GRR " +
            "JOIN GOV_ARTIFACT GA ON GRR.ARTIFACT_KEY = GA.ARTIFACT_KEY " +
            "WHERE GA.ARTIFACT_ID = ? AND GA.ARTIFACT_TYPE = ? AND GA.ORGANIZATION = ?";
    ...
    public static final String GET_RULE_VIOLATIONS =
            "SELECT DISTINCT GV.RULE_NAME, GV.VIOLATED_PATH, GRULE.SEVERITY " +
                    ...
                    "WHERE GA.ARTIFACT_ID = ? AND GA.ARTIFACT_TYPE = ? AND GA.ORGANIZATION = ? AND " +
                    "GRR.RULESET_ID = ?";
    ...
    public static final String GET_ALL_EVALUTED_ARTIFACTS = "SELECT DISTINCT GA.ARTIFACT_ID " +
    ...
    public static final String GET_NON_COMPLIANT_ARTIFACTS = "SELECT DISTINCT GA.ARTIFACT_ID " +
            "FROM GOV_ARTIFACT GA " +
            "JOIN GOV_POLICY_RUN GPR ON GA.ARTIFACT_KEY = GPR.ARTIFACT_KEY " +
            "JOIN GOV_RULESET_RUN GRR ON GA.ARTIFACT_KEY = GRR.ARTIFACT_KEY " +
            "WHERE GA.ARTIFACT_TYPE = ? AND GA.ORGANIZATION = ? AND GRR.RESULT = 0";

    public static final String GET_ARITFCATS_FOR_POLICY_RUN = "SELECT DISTINCT GA.ARTIFACT_ID, GA.ARTIFACT_TYPE " +
            "FROM GOV_ARTIFACT GA " +
            "JOIN GOV_POLICY_RUN GPR ON GA.ARTIFACT_KEY = GPR.ARTIFACT_KEY " +
            "WHERE GPR.POLICY_ID = ?";
