...
public class SQLConstants {
    ...
    public static final String GET_TEMPLATES_FOR_RULESET =
            "SELECT T.TEMPLATE_ID FROM GOV_TEMPLATE_RULESET_BINDING B " +
                    "JOIN GOV_DEVPORTAL_TEMPLATE T ON B.TEMPLATE_ID = T.TEMPLATE_ID " +
                    "WHERE B.RULESET_ID = ? AND T.ORGANIZATION = ?";
    ...
    public static final String CREATE_DEVPORTAL_TEMPLATE =
            "INSERT INTO GOV_DEVPORTAL_TEMPLATE (TEMPLATE_ID, NAME, DESCRIPTION, TAGS, ICON, FORM_CONFIG, " +
                    "FORM_CONFIG_HASH, STATUS, IS_DEFAULT, IS_GLOBAL, ORGANIZATION, CREATED_BY, CREATED_TIME) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    public static final String UPDATE_DEVPORTAL_TEMPLATE =
            "UPDATE GOV_DEVPORTAL_TEMPLATE SET NAME = ?, DESCRIPTION = ?, TAGS = ?, ICON = ?, " +
                    "FORM_CONFIG = ?, FORM_CONFIG_HASH = ?, STATUS = ?, IS_DEFAULT = ?, IS_GLOBAL = ?, " +
                    "UPDATED_BY = ?, LAST_UPDATED_TIME = ? WHERE TEMPLATE_ID = ? AND ORGANIZATION = ?";

    public static final String DELETE_DEVPORTAL_TEMPLATE =
            "DELETE FROM GOV_DEVPORTAL_TEMPLATE WHERE TEMPLATE_ID = ? AND ORGANIZATION = ?";

    public static final String GET_DEVPORTAL_TEMPLATE_BY_ID =
            "SELECT TEMPLATE_ID, NAME, DESCRIPTION, TAGS, ICON, FORM_CONFIG, FORM_CONFIG_HASH, STATUS, IS_DEFAULT, " +
                    "IS_GLOBAL, ORGANIZATION, CREATED_BY, CREATED_TIME, UPDATED_BY, LAST_UPDATED_TIME " +
                    "FROM GOV_DEVPORTAL_TEMPLATE WHERE TEMPLATE_ID = ? AND (ORGANIZATION = ? OR IS_GLOBAL = 1)";

    public static final String GET_DEVPORTAL_TEMPLATE_BY_NAME =
            "SELECT TEMPLATE_ID, NAME, DESCRIPTION, TAGS, ICON, FORM_CONFIG, FORM_CONFIG_HASH, STATUS, IS_DEFAULT, " +
                    "IS_GLOBAL, ORGANIZATION, CREATED_BY, CREATED_TIME, UPDATED_BY, LAST_UPDATED_TIME " +
                    "FROM GOV_DEVPORTAL_TEMPLATE WHERE NAME = ? AND ORGANIZATION = ?";

    public static final String GET_DEFAULT_DEVPORTAL_TEMPLATE =
            "SELECT TEMPLATE_ID, NAME, DESCRIPTION, TAGS, ICON, FORM_CONFIG, FORM_CONFIG_HASH, STATUS, IS_DEFAULT, " +
                    "IS_GLOBAL, ORGANIZATION, CREATED_BY, CREATED_TIME, UPDATED_BY, LAST_UPDATED_TIME " +
                    "FROM GOV_DEVPORTAL_TEMPLATE WHERE ORGANIZATION = ? AND IS_DEFAULT = 1 AND IS_GLOBAL = 0";

    public static final String GET_GLOBAL_DEFAULT_DEVPORTAL_TEMPLATE =
            "SELECT TEMPLATE_ID, NAME, DESCRIPTION, TAGS, ICON, FORM_CONFIG, FORM_CONFIG_HASH, STATUS, IS_DEFAULT, " +
                    "IS_GLOBAL, ORGANIZATION, CREATED_BY, CREATED_TIME, UPDATED_BY, LAST_UPDATED_TIME " +
                    "FROM GOV_DEVPORTAL_TEMPLATE WHERE IS_GLOBAL = 1 AND IS_DEFAULT = 1";

    public static final String GET_DEVPORTAL_TEMPLATES =
            "SELECT TEMPLATE_ID, NAME, DESCRIPTION, TAGS, ICON, FORM_CONFIG, FORM_CONFIG_HASH, STATUS, IS_DEFAULT, " +
                    "IS_GLOBAL, ORGANIZATION, CREATED_BY, CREATED_TIME, UPDATED_BY, LAST_UPDATED_TIME " +
                    "FROM GOV_DEVPORTAL_TEMPLATE WHERE ORGANIZATION = ? OR IS_GLOBAL = 1 ORDER BY IS_GLOBAL DESC, NAME";

    public static final String GET_ALL_DEVPORTAL_TEMPLATES =
            "SELECT TEMPLATE_ID, NAME, DESCRIPTION, TAGS, ICON, FORM_CONFIG, FORM_CONFIG_HASH, STATUS, IS_DEFAULT, " +
                    "IS_GLOBAL, ORGANIZATION, CREATED_BY, CREATED_TIME, UPDATED_BY, LAST_UPDATED_TIME " +
                    "FROM GOV_DEVPORTAL_TEMPLATE ORDER BY IS_GLOBAL DESC, ORGANIZATION, NAME";

    public static final String RESET_DEFAULT_DEVPORTAL_TEMPLATE =
            "UPDATE GOV_DEVPORTAL_TEMPLATE SET IS_DEFAULT = 0, UPDATED_BY = ?, LAST_UPDATED_TIME = ? " +
                    "WHERE ORGANIZATION = ? AND IS_GLOBAL = 0";

    public static final String RESET_DEFAULT_DEVPORTAL_TEMPLATE_EXCLUDING_ID =
            "UPDATE GOV_DEVPORTAL_TEMPLATE SET IS_DEFAULT = 0, UPDATED_BY = ?, LAST_UPDATED_TIME = ? " +
                    "WHERE ORGANIZATION = ? AND IS_GLOBAL = 0 AND TEMPLATE_ID <> ?";

    public static final String RESET_GLOBAL_DEFAULT_DEVPORTAL_TEMPLATE =
            "UPDATE GOV_DEVPORTAL_TEMPLATE SET IS_DEFAULT = 0, UPDATED_BY = ?, LAST_UPDATED_TIME = ? " +
                    "WHERE IS_GLOBAL = 1";

    public static final String RESET_GLOBAL_DEFAULT_DEVPORTAL_TEMPLATE_EXCLUDING_ID =
            "UPDATE GOV_DEVPORTAL_TEMPLATE SET IS_DEFAULT = 0, UPDATED_BY = ?, LAST_UPDATED_TIME = ? " +
                    "WHERE IS_GLOBAL = 1 AND TEMPLATE_ID <> ?";

    public static final String CREATE_TEMPLATE_RULESET_BINDING =
            "INSERT INTO GOV_TEMPLATE_RULESET_BINDING (BINDING_ID, TEMPLATE_ID, RULESET_ID, BINDING_ORDER, " +
                    "CREATED_BY, CREATED_TIME) VALUES (?, ?, ?, ?, ?, ?)";

    public static final String DELETE_TEMPLATE_RULESET_BINDINGS =
            "DELETE FROM GOV_TEMPLATE_RULESET_BINDING WHERE TEMPLATE_ID = ?";

    public static final String GET_TEMPLATE_RULESET_BINDINGS =
            "SELECT B.BINDING_ID, B.TEMPLATE_ID, B.RULESET_ID, B.BINDING_ORDER, B.CREATED_BY, B.CREATED_TIME, " +
                    "R.NAME AS RULESET_NAME, R.DESCRIPTION AS RULESET_DESCRIPTION, " +
                    "R.DOCUMENTATION_LINK AS DOCUMENTATION_LINK, R.RULE_TYPE, R.ARTIFACT_TYPE " +
                    "FROM GOV_TEMPLATE_RULESET_BINDING B " +
                    "LEFT JOIN GOV_RULESET R ON B.RULESET_ID = R.RULESET_ID " +
                    "WHERE B.TEMPLATE_ID = ? ORDER BY B.BINDING_ORDER";

    public static final String CREATE_TEMPLATE_RULESET_KM_SCOPE =
            "INSERT INTO GOV_TEMPLATE_RULESET_KM_SCOPE (BINDING_ID, KEY_MANAGER_UUID, ORGANIZATION) " +
                    "VALUES (?, ?, ?)";

    public static final String GET_TEMPLATE_RULESET_KM_SCOPES =
            "SELECT S.BINDING_ID, S.KEY_MANAGER_UUID, S.ORGANIZATION " +
                    "FROM GOV_TEMPLATE_RULESET_KM_SCOPE S " +
                    "JOIN GOV_TEMPLATE_RULESET_BINDING B ON S.BINDING_ID = B.BINDING_ID " +
                    "WHERE B.TEMPLATE_ID = ?";

    public static final String CHECK_RULESET_IN_ORGANIZATION =
            "SELECT RULESET_ID FROM GOV_RULESET WHERE RULESET_ID = ? AND ORGANIZATION = ?";

    public static final String DELETE_APP_GOVERNANCE_SNAPSHOT =
            "DELETE FROM GOV_APP_GOVERNANCE_SNAPSHOT WHERE APPLICATION_ID = ?";

    public static final String CREATE_APP_GOVERNANCE_SNAPSHOT =
            "INSERT INTO GOV_APP_GOVERNANCE_SNAPSHOT (SNAPSHOT_ID, APPLICATION_ID, APPLICATION_UUID, " +
                    "SOURCE_TEMPLATE_ID, TEMPLATE_NAME, TEMPLATE_DESCRIPTION, FORM_CONFIG, FORM_CONFIG_HASH, " +
                    "ORGANIZATION, CAPTURED_BY, CAPTURED_AT) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    public static final String GET_TEMPLATE_RULESET_SNAPSHOT_SOURCES =
            "SELECT B.BINDING_ID, B.RULESET_ID, B.BINDING_ORDER, R.NAME, R.DESCRIPTION, R.ARTIFACT_TYPE, " +
                    "R.RULE_TYPE, RC.CONTENT " +
                    "FROM GOV_TEMPLATE_RULESET_BINDING B " +
                    "JOIN GOV_RULESET R ON B.RULESET_ID = R.RULESET_ID " +
                    "JOIN GOV_RULESET_CONTENT RC ON R.RULESET_ID = RC.RULESET_ID " +
                    "WHERE B.TEMPLATE_ID = ? ORDER BY B.BINDING_ORDER";

    public static final String CREATE_APP_RULESET_SNAPSHOT =
            "INSERT INTO GOV_APP_RULESET_SNAPSHOT (SNAPSHOT_RULESET_ID, SNAPSHOT_ID, SOURCE_RULESET_ID, " +
                    "RULESET_NAME, RULESET_DESCRIPTION, ARTIFACT_TYPE, RULESET_TYPE, YAML_CONTENT, " +
                    "CONTENT_SHA256, BINDING_ORDER) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    public static final String GET_TEMPLATE_RULESET_SNAPSHOT_KM_SCOPES =
            "SELECT S.KEY_MANAGER_UUID, KM.NAME AS KEY_MANAGER_NAME " +
                    "FROM GOV_TEMPLATE_RULESET_KM_SCOPE S " +
                    "LEFT JOIN AM_KEY_MANAGER KM ON S.KEY_MANAGER_UUID = KM.UUID " +
                    "WHERE S.BINDING_ID = ?";

    public static final String CREATE_APP_RULESET_SNAPSHOT_KM_SCOPE =
            "INSERT INTO GOV_APP_RULE_SNAP_KM_SCOPE (SNAPSHOT_RULESET_ID, KEY_MANAGER_UUID, KEY_MANAGER_NAME) " +
                    "VALUES (?, ?, ?)";

    public static final String GET_APP_GOVERNANCE_SNAPSHOT =
            "SELECT SNAPSHOT_ID, APPLICATION_ID, APPLICATION_UUID, SOURCE_TEMPLATE_ID, TEMPLATE_NAME, " +
                    "TEMPLATE_DESCRIPTION, FORM_CONFIG, FORM_CONFIG_HASH, ORGANIZATION, CAPTURED_BY, CAPTURED_AT " +
                    "FROM GOV_APP_GOVERNANCE_SNAPSHOT WHERE APPLICATION_ID = ?";

    public static final String GET_APP_RULESET_SNAPSHOTS =
            "SELECT SNAPSHOT_RULESET_ID, SNAPSHOT_ID, SOURCE_RULESET_ID, RULESET_NAME, RULESET_DESCRIPTION, " +
                    "ARTIFACT_TYPE, RULESET_TYPE, YAML_CONTENT, CONTENT_SHA256, BINDING_ORDER " +
                    "FROM GOV_APP_RULESET_SNAPSHOT WHERE SNAPSHOT_ID = ? ORDER BY BINDING_ORDER";

    public static final String GET_APP_RULESET_SNAPSHOT_KM_SCOPES =
            "SELECT SNAPSHOT_RULESET_ID, KEY_MANAGER_UUID, KEY_MANAGER_NAME " +
                    "FROM GOV_APP_RULE_SNAP_KM_SCOPE WHERE SNAPSHOT_RULESET_ID = ?";
