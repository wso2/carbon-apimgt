...
public class SQLConstants {
    ...
    public static final String GET_FAILED_RULESET_RUNS =
    ...
    public static final String GET_FAILED_RULESET_RUNS_FOR_ARTIFACT =
            "SELECT DISTINCT GRR.RULESET_ID, GRULE.SEVERITY, GP.COMPLIANCE_AFFECTING_SEVERITIES "
                    + "FROM GOV_RULESET_RUN GRR "
                    + "JOIN GOV_ARTIFACT GA ON GRR.ARTIFACT_KEY = GA.ARTIFACT_KEY "
                    + POLICY_SEVERITY_JOINS
    ...
    /**
     * The same question asked of one policy rather than of every policy governing the artifact.
     * <p>
     * The queries above deliberately union the severities of every governing policy, which is right for a view
     * that has no policy in its path: a shared ruleset is violated if any policy judging it says so. A per policy
     * verdict is a different question, and answering it with that union reports a policy as violated because a
     * different policy counts a severity this one excluded. Scoping to the policy keeps its own selection as the
     * only one consulted.
     */
    public static final String GET_FAILED_RULESET_RUNS_FOR_ARTIFACT_AND_POLICY =
            "SELECT DISTINCT GRR.RULESET_ID, GRULE.SEVERITY, GP.COMPLIANCE_AFFECTING_SEVERITIES "
                    + "FROM GOV_RULESET_RUN GRR "
                    + "JOIN GOV_ARTIFACT GA ON GRR.ARTIFACT_KEY = GA.ARTIFACT_KEY "
                    + POLICY_SEVERITY_JOINS
     ...
     */
    public static final String GET_VIOLATED_POLICIES =
