...
CREATE TABLE IF NOT EXISTS GOV_ARTIFACT (
    ...
    ORGANIZATION VARCHAR(128) NOT NULL,
    CONSTRAINT GOV_ARTIFACT_CONSTRAINT UNIQUE (ARTIFACT_ID, ARTIFACT_TYPE, ORGANIZATION),
...
CREATE TABLE IF NOT EXISTS GOV_POLICY_RESULT (
    POLICY_RESULT_ID VARCHAR(36) NOT NULL,
    ARTIFACT_KEY VARCHAR(36) NOT NULL,
    POLICY_ID VARCHAR(36) NOT NULL,
    RESULT INT NOT NULL, -- 1 - Pass/Adhered, 0 - Fail/Not Adhered
    CONSTRAINT POLICY_RESULT UNIQUE (ARTIFACT_KEY, POLICY_ID),
    FOREIGN KEY (ARTIFACT_KEY) REFERENCES GOV_ARTIFACT(ARTIFACT_KEY),
    FOREIGN KEY (POLICY_ID) REFERENCES GOV_POLICY(POLICY_ID),
    PRIMARY KEY (POLICY_RESULT_ID)
) ENGINE=INNODB;

CREATE TABLE IF NOT EXISTS GOV_RULESET_RESULT (
    RULESET_RESULT_ID VARCHAR(36) NOT NULL,
    POLICY_RESULT_ID VARCHAR(36) NOT NULL,
    RULESET_ID VARCHAR(36) NOT NULL,
    RESULT INT NOT NULL, -- 1 - Pass, 0 - Fail
    RESULT_TIMESTAMP DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT RULESET_RESULT UNIQUE (POLICY_RESULT_ID, RULESET_ID),
    FOREIGN KEY (POLICY_RESULT_ID) REFERENCES GOV_POLICY_RESULT(POLICY_RESULT_ID),
    FOREIGN KEY (RULESET_ID) REFERENCES GOV_RULESET(RULESET_ID),
    PRIMARY KEY (RULESET_RESULT_ID)
) ENGINE=INNODB;
...
CREATE TABLE IF NOT EXISTS GOV_RULE_VIOLATION (
    ...
    FOREIGN KEY (RULESET_RESULT_ID) REFERENCES GOV_RULESET_RESULT(RULESET_RESULT_ID),
    FOREIGN KEY (RULESET_ID, RULE_NAME) REFERENCES GOV_RULESET_RULE(RULESET_ID, RULE_NAME),
    PRIMARY KEY (RULESET_RESULT_ID, RULE_NAME, VIOLATED_PATH)
) ENGINE=INNODB;
