...
public class ComplianceAffectingSeverityTest {
    ...
    @Test
    public void testAnInvalidSelectionIsNotConfusedWithAnUnavailableDeployment() {

        // Two different 400s live side by side on this branch: the deployment has not enabled the feature at all,
        // or the selection itself is unusable. A client has to be able to tell them apart, so the codes must differ.
        Assert.assertNotEquals("A bad request and an un-opted-in deployment cannot share one error code",
