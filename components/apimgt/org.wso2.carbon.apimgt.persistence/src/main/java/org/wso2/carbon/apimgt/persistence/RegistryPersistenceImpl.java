...
public class RegistryPersistenceImpl implements APIPersistence {
    ...
    public PublisherAPI getPublisherAPI(Organization org, String apiId) throws APIPersistenceException {
        //getAPIbyId(apiId, org.getName());//FIX==
    ...
    public DevPortalAPI getDevPortalAPI(Organization org, String apiId) throws APIPersistenceException {
        ...
        return new DevPortalAPI();
    }

    public Map<String, Object> searchPaginatedAPIs(String paginatedSearchQuery, Organization requestedOrg, int start,
...
//            if (documentIndexer != null && documentIndexer instanceof DocumentIndexer) {
    ...
    /*
    public Documentation getDocumentation(Organization requestedOrg, String apiId, String docId)
                                    throws APIManagementException {
        String requestedTenantDomain = requestedOrg.getName();
...
//    }
    ...
    @Override
    public PublisherAPI addAPI(Organization org, PublisherAPI api) throws APIPersistenceException {
        ...
        if (apiGenericArtifactManager == null) {
            String errorMessage = "Failed to retrieve artifact manager when creating API " + api.getApiName();
            log.error(errorMessage);
        ...
        try {
            ...
            GenericArtifact genericArtifact = apiGenericArtifactManager
                                            .newGovernanceArtifact(new QName(api.getApiName()));
            if (genericArtifact == null) {
                String errorMessage = "Generic artifact is null when creating API " + api.getApiName();
                log.error(errorMessage);
                throw new APIManagementException(errorMessage);
            }
            ...
            String providerPath = RegistryPersistenceUtil.getAPIProviderPath(api.getProviderName()); // TODO check email username conversion
           ...
           // // RegistryPersistenceUtil.attachLabelsToAPIArtifact(artifact, api, tenantDomain, gatewayLabelList);
            ...
            Set<String> publisherAccessControlRoles = api.getAccessControlRoles();
            updateRegistryResources(artifactPath, String.join(" ", publisherAccessControlRoles), api.getAccessControl(),
                                            api.getAdditionalProperties());
            RegistryPersistenceUtil.setResourcePermissions(api.getProviderName(), api.getVisibility(),
                                            visibleRoles, artifactPath, registry);

            registry.commitTransaction();
            api.setId(artifact.getId());
            transactionCommitted = true;

            if (log.isDebugEnabled()) {
                String logMessage = "API Name: " + api.getApiName() + ", API Version " + api
                                                .getVersion() + " created";
                log.debug(logMessage);
            }
            if (log.isDebugEnabled()) {
                log.debug("API details successfully added to the registry. API Name: " + api.getApiName()
                                                + ", API Version : " + api.getVersion() + ", API context : "
                                                + api.getContext());
            }
        } catch (RegistryException e) {
            try {
                ...
                log.error("Error while rolling back the transaction for API: " + api.getApiName(), re);
            }
            ...
            try {
                ...
                throw new APIPersistenceException("Error while rolling back the transaction for API: " + api.getApiName(), ex);
            }
        }
        return api;
    ...
    @Override
    public void saveWSDL(Organization org, String apiId,
            org.wso2.carbon.apimgt.persistence.dto.ResourceFile wsdlResourceFile) throws WSDLPersistenceException {
        // TODO Auto-generated method stub
    ...
    public String createWsdl(API api, InputStream wsdlContent, OMElement wsdlContentEle) {
        ...
        try {
            ...
            GenericArtifact artifact = null;   //GenericArtifact artifact = RegistryPersistenceUtil.createAPIArtifactContent(genericArtifact, api);////////comented
            String artifactPath = GovernanceUtils.getArtifactPath(registry, artifact.getId());
    ...
    public String updateWsdlFromWsdlFile(String apiId, ResourceFile wsdlResourceFile)
                                    throws APIManagementException {
        API api = null;
        try {
            ...
            //apiArtifact = RegistryPersistenceUtil.createAPIArtifactContent(genericArtifact, api);//////////Commented
