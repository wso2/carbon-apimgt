...
/**
 * Guardrail faulty request data collector.
 */
public class GuardrailFaultDataCollector extends AbstractFaultDataCollector {
    private static final Log log = LogFactory.getLog(GuardrailFaultDataCollector.class);
    private AnalyticsDataProvider provider;

    public GuardrailFaultDataCollector(AnalyticsDataProvider provider, FaultCategory subType,
            RequestDataPublisher processor) {
        super(provider, subType, processor);
    }

    public GuardrailFaultDataCollector(AnalyticsDataProvider provider) {
        this(provider, FaultCategory.GUARDRAIL_FAULT, new FaultyRequestDataPublisher());
    ...
    public void collectFaultData(Event faultyEvent) throws AnalyticsException {
        if (log.isDebugEnabled()) {
            log.debug("Handling guardrail failure analytics events");
        }
