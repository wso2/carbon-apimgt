...
UndeployApiApiService delegate = new UndeployApiApiServiceImpl();
    ...
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Uneploy the API in the gateway", notes = "This operation is used to undeploy an API in the gateway. If the Tenant domain is not provided carbon.super will be picked as the Tenant domain. ", response = DeployResponseDTO.class, tags={  })
    @ApiResponses(value = { 
