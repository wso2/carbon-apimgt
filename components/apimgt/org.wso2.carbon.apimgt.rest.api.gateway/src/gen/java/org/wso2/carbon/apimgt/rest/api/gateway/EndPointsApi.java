...
import org.wso2.carbon.apimgt.rest.api.gateway.impl.EndPointsApiServiceImpl;
...
EndPointsApiService delegate = new EndPointsApiServiceImpl();
    ...
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Get end-points from the storage for the API", notes = "This operation is used to get the end-points from the storage. If the Tenant domain is not provided carbon.super will be picked as the Tenant domain. ", response = EndpointsDTO.class, tags={  })
    @ApiResponses(value = { 
