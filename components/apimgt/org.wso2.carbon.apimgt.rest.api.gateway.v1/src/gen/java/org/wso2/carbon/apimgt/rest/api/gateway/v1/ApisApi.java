...
ApisApiService delegate = new ApisApiServiceImpl();
    ...
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Get the API meta information of an api by providing the api name, version and context.", notes = "This operation is used to get the subscription information of an API from storage. If the Tenant domain is not provided carbon.super will be picked as the Tenant domain. ", response = APIListDTO.class, tags={ "Get API Info" })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "OK. End-points successfully retrieved from the storage for the API. ", response = APIListDTO.class),
        @ApiResponse(code = 400, message = "Bad Request. Invalid request or validation error.", response = ErrorDTO.class),
        @ApiResponse(code = 404, message = "Not Found. The specified resource does not exist.", response = ErrorDTO.class) })
    public Response apisGet( @ApiParam(value = "Name of the API ")  @QueryParam("name") String name,  @ApiParam(value = "Context of the API ")  @QueryParam("context") String context,  @ApiParam(value = "version of the API ")  @QueryParam("version") String version,  @ApiParam(value = "Tenant Domain of the API ")  @QueryParam("tenantDomain") String tenantDomain) throws APIManagementException{
        return delegate.apisGet(name, context, version, tenantDomain, securityContext);
    }
}
