...
@SuppressWarnings("unused") public class APIProviderHostObject extends ScriptableObject {
    ...
    public static NativeObject jsFunction_login(Context cx, Scriptable thisObj, Object[] args, Function funObj)
        ...
        try {
            ...
            boolean authorized =
                    APIUtil.checkPermissionQuietly(usernameWithDomain, APIConstants.Permissions.API_CREATE) || APIUtil
    ...
    public static boolean jsFunction_manageAPI(Context cx, Scriptable thisObj, Object[] args, Function funObj)
        ...
        if (subscriptionAvailability != null && subscriptionAvailability
                .equals(APIConstants.SUBSCRIPTION_TO_SPECIFIC_TENANTS)) {
        ...
        if (registeredResource == null) {
            ...
            if (!isNewResourceRegistered) {
                handleException(
                        "APIResource registration is failed while adding the API- " + api.getId().getApiName() + "-"
    ...
    public static boolean jsFunction_updateAPIImplementation(Context cx, Scriptable thisObj, Object[] args,
        ...
        if (apiData.get("swagger", apiData) != null) {
            ...
            //Read URI Templates from swagger resource and set to api object
            Set<URITemplate> uriTemplates = definitionFromSwagger20
                    .getURITemplates(api, (String) apiData.get("swagger", apiData));
            api.setUriTemplates(uriTemplates);
        ...
        if (apiData.get("inSeqFile", apiData) != null) {
            ...
            ResourceFile inSeq = new ResourceFile(inSeqFile.getInputStream(),
                    inSeqFile.getJavaScriptFile().getContentType());
            ...
            String inSeqPath =
                    APIUtil.getSequencePath(api.getId(), "in") + RegistryConstants.PATH_SEPARATOR + inSeqFileName;
        ...
        if (apiData.get("outSeqFile", apiData) != null) {
            ...
            ResourceFile outSeq = new ResourceFile(outSeqFile.getInputStream(),
                    outSeqFile.getJavaScriptFile().getContentType());
            ...
            String outSeqPath =
                    APIUtil.getSequencePath(api.getId(), "out") + RegistryConstants.PATH_SEPARATOR + outSeqFileName;
    ...
    public static NativeObject jsFunction_getSwagger12Resource(Context cx, Scriptable thisObj, Object[] args,
        ...
        try {
            ...
            try {
                ...
                apiJSON = definitionFromSwagger20
                        .getAPIDefinition(apiId, registry); //apiProvider.getSwagger12Definition(apiId);
    ...
    public static boolean jsFunction_addAPI(Context cx, Scriptable thisObj, Object[] args, Function funObj)
        ...
        if (apiProvider.isDuplicateContextTemplate(contextVal)) {
            handleException(
                    "Error occurred while adding the API. A duplicate API context already exists for " + contextVal);
        }
        String context = contextVal.startsWith("/") ? contextVal : ("/" + contextVal);
    ...
    public static boolean jsFunction_updateAPI(Context cx, Scriptable thisObj, Object[] args, Function funObj)
        ...
        String context = contextVal.startsWith("/") ? contextVal : ("/" + contextVal);
        String providerDomain = MultitenantUtils.getTenantDomain(String.valueOf(apiData.get("provider", apiData)));
        if (!MultitenantConstants.SUPER_TENANT_DOMAIN_NAME.equalsIgnoreCase(providerDomain) && !context
                .contains("/t/" + providerDomain)) {
            //Create tenant aware context for API
            context = "/t/" + providerDomain + context;
        }
    ...
    public static boolean jsFunction_updateAPIStatus(Context cx, Scriptable thisObj, Object[] args, Function funObj)
        ...
        try {
            ...
            if (api != null) {
                ...
                if ((oldStatus.equals(APIStatus.CREATED) || oldStatus.equals(APIStatus.PROTOTYPED)) && newStatus
                    ...
                    if (deprecateOldVersions) {
                        ...
                        for (API oldAPI : apiList) {
                            if (oldAPI.getId().getApiName().equals(name) &&
                                ...
                                apiProvider
                                        .changeAPIStatus(oldAPI, APIStatus.DEPRECATED, currentUser, publishToGateway);
                            }
                        }
                    }
                }
    ...
    public static NativeArray jsFunction_getSubscriberCountByAPIVersions(Context cx, Scriptable thisObj, Object[] args,
            Function funObj) throws APIManagementException {
        NativeArray myn = new NativeArray(0);
        String providerName = null;
        String apiName = null;
    ...
    public static boolean jsFunction_addDocumentation(Context cx, Scriptable thisObj, Object[] args, Function funObj)
    ...
    public static boolean jsFunction_hasUserPermissions(Context cx, Scriptable thisObj, Object[] args, Function funObj)
        ...
        String username = (String) args[0];
        return APIUtil.checkPermissionQuietly(username, APIConstants.Permissions.API_CREATE) || APIUtil
                .checkPermissionQuietly(username, APIConstants.Permissions.API_PUBLISH);
    }
    ...
    public static void jsFunction_loadRegistryOfTenant(Context cx, Scriptable thisObj, Object[] args, Function funObj) {
        ...
        if (tenantDomain != null && !org.wso2.carbon.base.MultitenantConstants.SUPER_TENANT_DOMAIN_NAME
                .equals(tenantDomain)) {
            try {
                int tenantId = ServiceReferenceHolder.getInstance().getRealmService().
                        getTenantManager().getTenantId(tenantDomain);
    ...
    public static boolean jsFunction_updateDocumentation(Context cx, Scriptable thisObj, Object[] args, Function funObj)
            throws APIManagementException, ScriptException {
        if (args == null || args.length == 0) {
            handleException("Invalid number of parameters or their types.");
        }
            ...
            try {
                ...
                if (type != null && type.equals("wsdl")) {
                    ...
                    if (url.getProtocol().matches("https")) {
                        ...
                        headRequestResult
                                .put("isContainUriTemplatesOnly", headRequestResult, isContainUriTemplatesOnly);
    ...
    public static String jsFunction_getAPIStoreURL(Context cx, Scriptable thisObj, Object[] args, Function funObj)
        ...
        if (args != null && args.length > 0 && args[0] != null) {
            String tenantDomain = args[0].toString();
            APIProvider apiProvider = getAPIProvider(thisObj);
            Map<String, String> domains = apiProvider
                    .getTenantDomainMappings(tenantDomain, APIConstants.API_DOMAIN_MAPPINGS_STORE);
    ...
    public static boolean jsFunction_showAPIStoreURL(Context cx, Scriptable thisObj, Object[] args, Function funObj) {
    ...
    private static NativeObject sendHttpHEADRequest(String urlVal, String invalidStatusCodesRegex) {
        ...
        if (System.getProperty(APIConstants.HTTP_PROXY_HOST) != null
            ...
            client.getParams()
                    .setParameter(ConnRoutePNames.DEFAULT_PROXY, new HttpHost(proxyHost, Integer.parseInt(proxyPort)));
        }
        ...
        try {
            ...
            if (errorMsg.length > 1) {
                response = errorMsg[errorMsg.length
                        - 1]; //This is to get final readable part of the error message in the exception and send to the client
                isConnectionError = false;
            }
        } finally {
	...
	*/
     ...
     * @throws APIManagementException Wrapped exception by org.wso2.carbon.apimgt.api.APIManagementException
     */
    public static NativeObject jsFunction_getDocument(Context cx, Scriptable thisObj, Object[] args, Function funObj)
