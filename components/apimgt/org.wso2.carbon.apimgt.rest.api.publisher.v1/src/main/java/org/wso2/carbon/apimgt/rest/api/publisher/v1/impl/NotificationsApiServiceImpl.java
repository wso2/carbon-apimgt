...
public class NotificationsApiServiceImpl implements NotificationsApiService {
    ...
    public Response getNotifications(String sortOrder, Integer limit, Integer offset, MessageContext messageContext) throws APIManagementException {
        ...
        try {
            ...
            notificationList = apiProvider.getNotifications(username, organization, sortOrder, limit, offset);
    ...
    @Override
    public Response markNotificationAsReadById(String notificationId, PatchNotificationRequestDTO patchNotificationRequestDTO,
    ...
    public Response deleteNotifications(MessageContext messageContext) throws APIManagementException {
        ...
        try {
            String organization = RestApiUtil.getValidatedOrganization(messageContext);
            APIProvider apiProvider = RestApiCommonUtil.getLoggedInUserProvider();
            boolean result = apiProvider.deleteAllNotifications(username, organization);
            if(!result){
                return Response.status(Response.Status.NOT_FOUND).build();
            }
        } catch (APIManagementException e) {
