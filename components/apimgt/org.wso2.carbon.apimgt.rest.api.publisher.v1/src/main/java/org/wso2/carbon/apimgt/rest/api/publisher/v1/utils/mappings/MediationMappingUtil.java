...
public class MediationMappingUtil {
    ...
    public static MediationDTO fromMediationToDTO(Mediation mediation) {
        ...
        for (MediationDTO.SourceTypeEnum DTOenum : MediationDTO.SourceTypeEnum.values()) {
            if (mediation.getSourceType().name().equalsIgnoreCase(DTOenum.name())) {
                mediationDTO.setSourceType(DTOenum);
            }
        }
        return mediationDTO;
    }
