...
ThrottlingPoliciesApiService delegate = new ThrottlingPoliciesApiServiceImpl();
    ...
    @Produces({ "application/json" })
    @ApiOperation(value = "Get streaming throttling policies", notes = "This operation can be used to list the available streaming subscription policies ", response = SubscriptionPolicyDTO.class, authorizations = {
        @Authorization(value = "OAuth2Security", scopes = {
            @AuthorizationScope(scope = "apim:api_view", description = "View API")
        })
    }, tags={ "Throttling Policies",  })
    @ApiResponses(value = { 
        ...
        @ApiResponse(code = 304, message = "Not Modified. Empty body because the client has already the latest version of the requested resource (Will be supported in future). ", response = Void.class),
        @ApiResponse(code = 406, message = "Not Acceptable. The requested media type is not supported.", response = ErrorDTO.class) })
    public Response getSubscriptionThrottlingPolicies( @ApiParam(value = "Validator for conditional requests; based on the ETag of the formerly retrieved variant of the resource. " )@HeaderParam("If-None-Match") String ifNoneMatch) throws APIManagementException{
        return delegate.getSubscriptionThrottlingPolicies(ifNoneMatch, securityContext);
    }
