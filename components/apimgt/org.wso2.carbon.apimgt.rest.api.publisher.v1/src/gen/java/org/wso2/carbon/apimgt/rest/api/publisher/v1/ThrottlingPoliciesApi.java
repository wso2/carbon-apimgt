...
import org.wso2.carbon.apimgt.rest.api.publisher.v1.dto.ThrottlePolicyResetDTO;
...
ThrottlingPoliciesApiService delegate = new ThrottlingPoliciesApiServiceImpl();
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "OK. List of subscription policies returned. ", response = SubscriptionPolicyListDTO.class),
        @ApiResponse(code = 304, message = "Not Modified. Empty body because the client has already the latest version of the requested resource (Will be supported in future). ", response = Void.class),
        @ApiResponse(code = 406, message = "Not Acceptable. The requested media type is not supported.", response = ErrorDTO.class) })
    public Response getSubscriptionThrottlingPolicies( @ApiParam(value = "Maximum size of resource array to return. ", defaultValue="25") @DefaultValue("25") @QueryParam("limit") Integer limit,  @ApiParam(value = "Starting point within the complete list of items qualified. ", defaultValue="0") @DefaultValue("0") @QueryParam("offset") Integer offset,  @ApiParam(value = "Validator for conditional requests; based on the ETag of the formerly retrieved variant of the resource. " )@HeaderParam("If-None-Match") String ifNoneMatch) throws APIManagementException{
        return delegate.getSubscriptionThrottlingPolicies(limit, offset, ifNoneMatch, securityContext);
    }
    ...
    @Produces({ "application/json" })
    @ApiOperation(value = "Reset Subscription/API/Resource Throttle Policy", notes = "This operation can be used to reset the throttle policy for a specific subscription, api or resource tier. ", response = Void.class, authorizations = {
        @Authorization(value = "OAuth2Security", scopes = {
            @AuthorizationScope(scope = "apim:subscription_manage", description = "Manage all Subscription related operations")
        })
    }, tags={ "Throttling Policies" })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "OK. Throttle policy reset successfully", response = Void.class),
    ...
    public Response throttlingPoliciesResetPost(@ApiParam(value = "Payload for which the throttle policy needs to be reset " ) ThrottlePolicyResetDTO throttlePolicyResetDTO) throws APIManagementException{
        return delegate.throttlingPoliciesResetPost(throttlePolicyResetDTO, securityContext);
    }
}
