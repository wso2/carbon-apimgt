...
package org.wso2.carbon.apimgt.impl.gatewayBridge.dao;
...
public class WebhooksDAO {
     ...
     *
     * @param subscriberName the external gateways name
     * @return a list of subscriber DTOs.
     */
    public List<WebhookSubscriptionDTO> getSubscriptionsList(String subscriberName) throws APIManagementException {
        ...
        try (Connection conn = APIMgtDBUtil.getConnection()) {
            try (PreparedStatement preparedStatement = conn
                    .prepareStatement(SQLConstants.ExternalGatewayWebhooksSqlConstants.GET_SUBSCRIPTIONS_BY_NAME)) {
                preparedStatement.setString(1, subscriberName);
                try (ResultSet rs = preparedStatement.executeQuery()) {
                    if (rs.next()) {
