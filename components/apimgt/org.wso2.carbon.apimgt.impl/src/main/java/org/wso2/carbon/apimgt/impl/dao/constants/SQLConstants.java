  ...
  //--------------------
    ...
    public static final String GET_API_API_KEY_SQL =
            "SELECT K.API_KEY_UUID, AM.APPLICATION_UUID, APP.NAME AS APPLICATION_NAME, K.NAME, K.KEY_TYPE, K.TIME_CREATED, " +
                    ...
                    "WHERE KM.API_UUID = ? AND K.AUTHZ_USER = ? AND K.STATUS = 'ACTIVE' AND ((APP.ORGANIZATION IS NOT NULL AND APP.ORGANIZATION = ?) " +
    ...
    public static final String REMOVE_API_KEY_ASSOCIATION_SQL =
            "DELETE FROM AM_API_KEY_APPLICATION_MAPPING WHERE API_KEY_UUID = ? AND APPLICATION_UUID IN " +
                    "(SELECT A.UUID FROM AM_APPLICATION A WHERE A.ORGANIZATION = ?)";
