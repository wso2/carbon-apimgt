...
public class SQLConstants {
  ...
  //--------------------
    ...
    // Oracle (pre-23c) rejects LOB columns in UNION/UNION ALL (ORA-00932). The union branches below
    // project only scalar columns (including API_KEY_UUID); the BLOB API_KEY_PROPERTIES is fetched
    // once in the outer SELECT by joining the union result back to AM_API_KEY on API_KEY_UUID.
    public static final String GET_ALL_API_KEYS_SQL =
            "SELECT K.API_KEY_UUID AS API_KEY_UUID, K.API_KEY_HASH AS API_KEY_HASH, K.NAME AS KEY_NAME, " +
                    "K.KEY_TYPE AS KEY_TYPE, K.AUTHZ_USER AS AUTHZ_USER, K.STATUS AS STATUS, " +
                    "K.VALIDITY_PERIOD AS VALIDITY_PERIOD, K.LAST_USED AS LAST_USED, " +
                    "K.TIME_CREATED AS TIME_CREATED, K.API_KEY_PROPERTIES AS ADDITIONAL_PROPERTIES, " +
                    "U.APP_ID, U.API_ID, U.APPLICATION_UUID, U.API_UUID, U.APPLICATION_NAME, U.API_NAME " +
                    "FROM AM_API_KEY K INNER JOIN (" +
                    "SELECT KEY_TBL.API_KEY_UUID AS API_KEY_UUID, NULL AS APP_ID, API_TBL.API_ID AS API_ID, " +
                    "NULL AS APPLICATION_UUID, API_TBL.API_UUID AS API_UUID, NULL AS APPLICATION_NAME, " +
                    "API_TBL.API_NAME AS API_NAME " +
                    "FROM AM_API_KEY KEY_TBL INNER JOIN AM_API_KEY_API_MAPPING KEY_API_MAP " +
                    "ON KEY_TBL.API_KEY_UUID = KEY_API_MAP.API_KEY_UUID INNER JOIN AM_API API_TBL " +
                    "ON KEY_API_MAP.API_UUID = API_TBL.API_UUID LEFT OUTER JOIN AM_API_KEY_APPLICATION_MAPPING " +
                    "KEY_APP_MAP ON KEY_TBL.API_KEY_UUID = KEY_APP_MAP.API_KEY_UUID " +
                    "WHERE API_TBL.ORGANIZATION = ? AND KEY_TBL.STATUS = 'ACTIVE' " +
                    "AND KEY_APP_MAP.APPLICATION_UUID IS NULL " +
                    "UNION ALL " +
                    "SELECT KEY_TBL.API_KEY_UUID AS API_KEY_UUID, APP_TBL.APPLICATION_ID AS APP_ID, " +
                    "NULL AS API_ID, APP_TBL.UUID AS APPLICATION_UUID, NULL AS API_UUID, " +
                    "APP_TBL.NAME AS APPLICATION_NAME, NULL AS API_NAME " +
                    "FROM AM_API_KEY KEY_TBL INNER JOIN AM_API_KEY_APPLICATION_MAPPING KEY_APP_MAP " +
                    "ON KEY_TBL.API_KEY_UUID = KEY_APP_MAP.API_KEY_UUID INNER JOIN AM_APPLICATION APP_TBL " +
                    "ON KEY_APP_MAP.APPLICATION_UUID = APP_TBL.UUID LEFT OUTER JOIN AM_API_KEY_API_MAPPING " +
                    "KEY_API_MAP ON KEY_TBL.API_KEY_UUID = KEY_API_MAP.API_KEY_UUID " +
                    "WHERE APP_TBL.ORGANIZATION = ? AND KEY_TBL.STATUS = 'ACTIVE' " +
                    "AND KEY_API_MAP.API_UUID IS NULL " +
                    "UNION ALL " +
                    "SELECT KEY_TBL.API_KEY_UUID AS API_KEY_UUID, APP_TBL.APPLICATION_ID AS APP_ID, " +
                    "API_TBL.API_ID AS API_ID, APP_TBL.UUID AS APPLICATION_UUID, API_TBL.API_UUID AS API_UUID, " +
                    "APP_TBL.NAME AS APPLICATION_NAME, API_TBL.API_NAME AS API_NAME " +
                    "FROM AM_API_KEY KEY_TBL INNER JOIN AM_API_KEY_API_MAPPING KEY_API_MAP " +
                    "ON KEY_TBL.API_KEY_UUID = KEY_API_MAP.API_KEY_UUID INNER JOIN AM_API API_TBL " +
                    "ON KEY_API_MAP.API_UUID = API_TBL.API_UUID INNER JOIN AM_API_KEY_APPLICATION_MAPPING " +
                    "KEY_APP_MAP ON KEY_TBL.API_KEY_UUID = KEY_APP_MAP.API_KEY_UUID INNER JOIN AM_APPLICATION " +
                    "APP_TBL ON KEY_APP_MAP.APPLICATION_UUID = APP_TBL.UUID " +
                    "WHERE API_TBL.ORGANIZATION = ? AND APP_TBL.ORGANIZATION = ? AND KEY_TBL.STATUS = 'ACTIVE'" +
                    ") U ON K.API_KEY_UUID = U.API_KEY_UUID " +
