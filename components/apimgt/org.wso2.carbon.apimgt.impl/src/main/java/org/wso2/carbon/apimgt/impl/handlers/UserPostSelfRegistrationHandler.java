...
public class UserPostSelfRegistrationHandler extends AbstractEventHandler {
    ...
    private void executeUserRegistrationWorkflow(String tenantDomain, String userName)
        ...
        try {
            ...
            if (UserManagerHolder.getUserManager().isExistingUser(IdentityTenantUtil.getTenantId(tenantDomain), userName)) {
                List<String> roleList = asList(UserManagerHolder.getUserManager()
                        .getRoleListOfUser(IdentityTenantUtil.getTenantId(tenantDomain), userName));
        ...
        } catch (WorkflowException | UserException e) {
            throw Utils
