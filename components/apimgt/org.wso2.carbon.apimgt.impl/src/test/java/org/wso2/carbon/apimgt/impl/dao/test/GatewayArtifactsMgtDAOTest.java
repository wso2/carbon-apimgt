...
public class GatewayArtifactsMgtDAOTest {
    ...
    @Test
    public void testGetAllGatewayAPIArtifactAndMetaData() throws APIManagementException {
        String uuid = UUID.randomUUID().toString();
        String name = "apiname";
        String version = "1.0.0";
        String revision = UUID.randomUUID().toString();
        ...
        String name2 = "apiname";
        String version2 = "1.0.0";
        String organization2 = "7a27a0db-2488-4d0f-8b8e-c4e6e74282d7";
        String revision2 = UUID.randomUUID().toString();
        URL resource2 = getClass().getClassLoader().getResource("admin-PizzaShackAPI-1.0.0.zip");
        File file2 = new File(resource2.getPath());
        gatewayArtifactsMgtDAO.addGatewayAPIArtifactAndMetaData(uuid2, name2, version2, revision2, organization2,
                APIConstants.HTTP_PROTOCOL, file2);

        API api2 = new API(new APIIdentifier("test-provider2", name2, version2));
        api.setContext("/context2");
        api.setContextTemplate("/context2/{version2}");
        api.setUUID(uuid2);
        apiMgtDAO.addAPI(api2, -1234, organization2);

        String gatewayAPIId2 = gatewayArtifactsMgtDAO.getGatewayAPIId(name2, version2, organization2);
        Assert.assertEquals(gatewayAPIId2, uuid2);
        Map<String, String> gatewayVhosts2 = new HashMap<>();
        gatewayVhosts.put("label1", "dev.wso2.com");
        gatewayArtifactsMgtDAO.addAndRemovePublishedGatewayLabels(uuid2, revision2, Collections.asSet("label1"),
                gatewayVhosts2);
        List<APIRuntimeArtifactDto> artifacts2 = gatewayArtifactsMgtDAO.retrieveGatewayArtifactsByAPIIDAndLabel(uuid2,
                new String[]{"label1"}, organization2);
        Assert.assertEquals(artifacts2.size(), 1);
        RuntimeArtifactDto artifact2 = artifacts2.get(0);
        Assert.assertNotNull(artifact2);

        List<APIRuntimeArtifactDto> artifactsAll1 = gatewayArtifactsMgtDAO.retrieveAllGatewayArtifactsByAPIIDAndLabel(uuid2,
                new String[]{"label1"});
        Assert.assertEquals(artifactsAll1.size(), 1);
        List<APIRuntimeArtifactDto> artifactsAll2 = gatewayArtifactsMgtDAO.retrieveAllGatewayArtifactsByAPIIDAndLabel(uuid2,
                new String[]{"label1"});
        Assert.assertEquals(artifactsAll2.size(), 1);

        List<APIRuntimeArtifactDto> artifactsAll3 = gatewayArtifactsMgtDAO.retrieveAllGatewayArtifactsByLabel(new String[]{"label1"});
        Assert.assertEquals(artifactsAll3.size(), 2);

        List<APIRuntimeArtifactDto> artifactsAll4 = gatewayArtifactsMgtDAO.retrieveAllGatewayArtifacts();
        Assert.assertEquals(artifactsAll4.size(), 2);
