...
public class AbstractAPIManagerTestCase {
    ...
    public void testGetAllApis() throws GovernanceException, APIManagementException {
        ...
        Mockito.when(genericArtifactManager.getAllGenericArtifacts()).thenThrow(RegistryException.class).thenReturn
                (genericArtifacts);
    ...
    public void testGetApi()
        ...
        Mockito.when(tenantManager.getTenantId(SAMPLE_TENANT_DOMAIN)).thenThrow(UserStoreException.class).thenReturn
                (-1234);
        try {
        ...
        GenericArtifact genericArtifact = getGenericArtifact(SAMPLE_API_NAME, API_PROVIDER, SAMPLE_API_VERSION,"sample");
        Mockito.when(genericArtifactManager.getGenericArtifact(SAMPLE_RESOURCE_ID)).thenReturn
                (genericArtifact);
    ...
    public void testGetAPIbyUUID()
        ...
        TenantManager tenantManager = Mockito.mock(TenantManager.class);
        Mockito.when(tenantManager.getTenantId("test")).thenThrow(UserStoreException.class).thenReturn
                (-1234);
        ...
        GenericArtifact genericArtifact = getGenericArtifact(SAMPLE_API_NAME, API_PROVIDER, SAMPLE_API_VERSION,"sample");
        Mockito.when(genericArtifactManager.getGenericArtifact("1")).thenThrow(RegistryException.class).thenReturn(null,
                genericArtifact);
    ...
    public void testGetWsdl() throws APIManagementException, RegistryException, IOException {
        ...
        AbstractAPIManager abstractAPIManager = new AbstractAPIManagerWrapper(registry);
        APIIdentifier identifier = getAPIIdentifier(SAMPLE_API_NAME, API_PROVIDER, SAMPLE_API_VERSION);
        String wsdlName = identifier.getProviderName() + "--" + identifier.getApiName() +
                identifier.getVersion() + ".wsdl";
    ...
    public void testGetSwagger20Definition() throws Exception {
        ...
        TenantManager tenantManager = Mockito.mock(TenantManager.class);
        AbstractAPIManager abstractAPIManager = new AbstractAPIManagerWrapper(null,registryService,
                tenantManager);
        Mockito.when(tenantManager.getTenantId(SAMPLE_TENANT_DOMAIN)).thenThrow(UserStoreException.class).thenReturn
                (tenantId);
        ...
        abstractAPIManager.tenantDomain = SAMPLE_TENANT_DOMAIN;
        Assert.assertEquals(abstractAPIManager.getSwagger20Definition(identifier),swaggerContent);
        Mockito.when(registryService.getGovernanceUserRegistry(CarbonConstants.REGISTRY_ANONNYMOUS_USERNAME,
                tenantId)).thenThrow(RegistryException.class);
    ...
    public void testGetAllDocumentation() throws APIManagementException, RegistryException {
        ...
        String apiResourcePath = APIConstants.API_ROOT_LOCATION + RegistryConstants.PATH_SEPARATOR +
                ...
                identifier.getApiName() + RegistryConstants.PATH_SEPARATOR +
                identifier.getVersion() + APIConstants.API_RESOURCE_NAME;
        Mockito.when(registry.getAssociations(apiResourcePath, APIConstants.DOCUMENTATION_ASSOCIATION)).thenThrow
                (RegistryException.class).thenReturn(associations);
        try {
        ...
        String contentPath = APIConstants.API_LOCATION + RegistryConstants.PATH_SEPARATOR +
                identifier.getProviderName() + RegistryConstants.PATH_SEPARATOR +
                identifier.getApiName() + RegistryConstants.PATH_SEPARATOR +
                identifier.getVersion() + RegistryConstants.PATH_SEPARATOR +
                APIConstants.DOC_DIR + RegistryConstants.PATH_SEPARATOR + RegistryConstants.PATH_SEPARATOR + documentationName;
    ...
    public void testGetAllDocumentationWithLoggedUser()
        ...
        Mockito.when(registryService.getGovernanceUserRegistry(CarbonConstants.REGISTRY_ANONNYMOUS_USERNAME,
                tenantId)).thenThrow(RegistryException.class);
        Mockito.when(registry.getAssociations(apiResourcePath, APIConstants.DOCUMENTATION_ASSOCIATION)).thenReturn
                (associations);
        try {
            abstractAPIManager.getAllDocumentation(identifier, loggedInUser);
            Assert.fail("Registry exception not thrown for error scenario");
    ...
    public void testGetDocumentation() throws APIManagementException, RegistryException {
        ...
        Assert.assertTrue(abstractAPIManager.getDocumentation(identifier, DocumentationType.OTHER, documentationName)
                .getId().equals(genericArtifact.getId()));
    }
