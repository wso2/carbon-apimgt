...
public class SynapseArtifactGenerator implements GatewayArtifactGenerator {
    ...
    private static final long KEEP_ALIVE_TIME_MS = 60000;
    private static final int QUEUE_CAPACITY = 1000;

    private ExecutorService artifactThreadPoolExecutor;
    ...
    public RuntimeArtifactDto generateGatewayArtifact(List<APIRuntimeArtifactDto> apiRuntimeArtifactDtoList)
        ...
        ConcurrentLinkedQueue<String> synapseArtifacts = new ConcurrentLinkedQueue<>();
        ConcurrentLinkedQueue<String> failedApis = new ConcurrentLinkedQueue<>();

        CountDownLatch latch = new CountDownLatch(apiRuntimeArtifactDtoList.size());
        ...
        if (artifactThreadPoolExecutor == null || artifactThreadPoolExecutor.isShutdown()) {
            throw new APIManagementException("SynapseArtifactGenerator executor service is not available");
        }

        for (APIRuntimeArtifactDto runTimeArtifact : apiRuntimeArtifactDtoList) {
            artifactThreadPoolExecutor.submit(() -> {
                try {
                                ...
                                try (InputStream artifact = (InputStream) runTimeArtifact.getArtifact()) {
                                    ...
                                    try {
                                        ...
                                        if (gatewayAPIDTO != null) {
                                            ...
                                            failedApis.add(
                                                    result.name + " (" + result.apiId + "): " + result.errorMessage);
                                        }
                                    } finally {
        ...
        try {
            log.info("Waiting for " + apiRuntimeArtifactDtoList.size() +
                    " API artifact processing tasks to complete...");

            if (!latch.await(5, TimeUnit.MINUTES)) {
                log.warn("Timeout reached while waiting for " + apiRuntimeArtifactDtoList.size() +
                        " API processing tasks to complete. Some tasks may still be running in background.");
                throw new APIManagementException("Processing timeout: " +
                        "Not all tasks completed within the timeout period");
            }

            log.info("All " + apiRuntimeArtifactDtoList.size() +
                    " API artifact processing tasks completed successfully");

        } catch (InterruptedException e) {
            log.error("Gateway artifact generation was interrupted while waiting for " +
                    apiRuntimeArtifactDtoList.size() + " tasks to complete", e);
            Thread.currentThread().interrupt();
            throw new APIManagementException("Gateway artifact generation was interrupted", e);
        }

        // Convert concurrent collections to regular lists
        List<String> synapseArtifactsList = new ArrayList<>(synapseArtifacts);
        List<String> failedApisList = new ArrayList<>(failedApis);

        if (!failedApisList.isEmpty()) {
            log.warn("Error while creating Synapse configurations for APIs " +
                    String.join(", ", failedApisList));
        }

        runtimeArtifactDto.setFile(false);
        runtimeArtifactDto.setArtifact(synapseArtifactsList);
