 ...
 * <p>
 * The field carries three meanings on a response and each one has to survive a round trip. Null means the
 * deployment has not enabled the feature and the control must not be offered at all. An empty string means the
 * feature is available but this policy counts every severity. A value lists the severities that decide the
 * verdict. On a request, null instead means the field was not sent and whatever is stored is preserved, while an
 * empty string clears the selection — getting either side wrong either hides a working feature, advertises one
 * that stores nothing, or silently drops a selection the deployment cannot honour.
 * <p>
...
public class PolicySeverityContractTest {
     ...
     *
     * @param stored Value held in the column, null when nothing is configured
     */
    ...
    public void testAnUnknownSeverityIsRejectedBeforeThePolicyIsUpdated() throws Exception {
        ...
        try {
            ...
            Assert.assertEquals("The caller has to be told the selection is unusable, not that the deployment "
                            + "cannot store one",
                    APIMGovExceptionCodes.INVALID_COMPLIANCE_AFFECTING_SEVERITIES.getErrorCode(),
    ...
    @Test
    public void testAnUnknownSeverityIsRejectedEvenWhileTheFeatureIsOff() throws Exception {

        // With the feature off both refusals are possible. The request is unusable whatever the deployment can
        // store, so it is the request that is reported, and the policy is left alone either way.
        Mockito.when(policyManager.isComplianceAffectingSeverityFilteringEnabled()).thenReturn(false);
        try {
            policiesApiService.updateGovernancePolicyById(POLICY_ID, payload("ERROR,BLOCKER"), messageContext);
            Assert.fail("An unknown severity must not be accepted while the feature is off either");
        } catch (APIMGovernanceException e) {
            Assert.assertEquals(400, e.getErrorHandler().getHttpStatusCode());
        }
