...
public class JMSTaskManager {
    ...
    /**
     * Should this adapter use JMS 1.1 ? (when false, defaults to 1.0.2b)
     */
    ...
    private class MessageListenerTask implements Runnable, ExceptionListener {
            ...
            if (isActive()) {
                if (connection != null && cacheLevel < JMSConstants.CACHE_CONNECTION) {
                    try {
                        ...
                        log.warn("Error pausing Message Listener task for " + jmsConsumerName);
                    }
                }
        ...
        public void resume() {
            if (connection != null && cacheLevel < JMSConstants.CACHE_CONNECTION) {
                try {
                    ...
                    log.warn("Error resuming Message Listener task for " + jmsConsumerName);
                }
            }
        ...
        private Message receiveMessage() {
            ...
            try {
                ...
                logError("Error receiving message for " + jmsConsumerName, e);
            }
            return null;
        }
        ...
        private Session createSession() {
            try {
                if (log.isDebugEnabled()) {
                    log.debug("Creating a new JMS Session for " + jmsConsumerName);
                }
        ...
        private MessageConsumer createConsumer() {
            try {
                if (log.isDebugEnabled()) {
                    log.debug("Creating a new JMS MessageConsumer for " + jmsConsumerName);
                }
                ...
                handleException("Error creating JMS consumer for " + jmsConsumerName, e);
            }
            return null;
        }
    }
    ...
    /**
     * Get the InitialContext for lookup using the JNDI parameters
     *
    ...
    private UserTransaction getUserTransaction() {
        if (!cacheUserTransaction) {
            if (log.isDebugEnabled()) {
                log.debug("Acquiring a new UserTransaction for " + jmsConsumerName);
            }
    ...
    // -------------- getters and setters ------------------
    public String getJmsConsumerName() {
        return jmsConsumerName;
    }

    public void setJmsConsumerName(String jmsConsumerName) {
        this.jmsConsumerName = jmsConsumerName;
    }
