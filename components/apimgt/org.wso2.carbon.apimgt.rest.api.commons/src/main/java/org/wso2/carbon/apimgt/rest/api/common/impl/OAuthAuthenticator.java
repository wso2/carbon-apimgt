 ...
 */
public class OAuthAuthenticator implements RESTAPIAuthenticator {
    ...
    * */
    ...
    public boolean authenticate(Request request, Response responder, ServiceMethodInfo serviceMethodInfo)
        ...
        if (headers != null && headers.contains(RestApiConstants.COOKIE_HEADER)) {
            ...
            Map<String, String> tokenInfo = validateToken(cookies);
     ...
     *
     * @param cookies Authorization Bearer header which contains the access token
     * @return responseData if the token is a valid token
     */
    private Map<String, String> validateToken(String cookies) throws APIMgtSecurityException {
        // 1. Check whether this token is bearer token, if not return false
        String accessToken = extractAccessToken(cookies);
    ...
    * */
    ...
    /**
     * @param cookie Authorization Bearer header which contains the access token
     * @return access token
     * @throws APIMgtSecurityException if the Authorization header is invalid
     */
    private String extractAccessToken(String cookie) throws APIMgtSecurityException {
        cookie = cookie.trim();
        String[] cookies = cookie.split(";");
        if (cookies.length >= 2) {
            return "1".contains(cookies[0].split("=")[0]) ?
