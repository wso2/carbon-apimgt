...
public class PerAPILogHandler {
     ...
     * @param messageContext MessageContext of the request
     */
    private static synchronized void logHeaders(String logID, String flow, String apiTo, String verb,
     ...
     * @param messageContext MessageContext of the request
     */
    private static synchronized void logPayload(String logID, String flow, String apiTo, String verb,
            MessageContext messageContext) {
        String payload;
        if (flow.contains("in")) {
     ...
     */
    private static synchronized void logEntity(String logID, String flow, String verb, String apiTo, String type, Object entry) {
        log.debug("logging the entity of the API request " + apiTo + "flow " + flow + " entity " + entry);
     ...
     */
    private static synchronized Map getTransportHeaders(org.apache.synapse.MessageContext messageContext) {
     ...
     */
    private static synchronized String collectPayload(MessageContext messageContext) {
        String payload = null;
        try {
     ...
     */
    private static synchronized String getBuiltPayload(MessageContext messageContext) {
        ...
        try {
            ...
            try {
                ...
                // continue the flow
                log.error("Could not replace faulty SOAP Envelop. Error: " + ex.getLocalizedMessage());
                return sb.toString();
            }
        }
        return sb.toString();
