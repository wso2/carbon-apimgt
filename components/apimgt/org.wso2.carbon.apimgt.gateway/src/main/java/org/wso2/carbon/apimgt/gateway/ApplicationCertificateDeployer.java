...
/**
 * This class used to deploy Application Certificates into Gateway.
 */
public class ApplicationCertificateDeployer {

    private static final Log log = LogFactory.getLog(ApplicationCertificateDeployer.class);
    ...
    public void deployCertificatesAtStartup() throws APIManagementException {
        ...
        String endpoint = baseURL + APIConstants.APPLICATION_CERTIFICATE_RETRIEVAL;
    ...
    private void retrieveCertificatesAndDeploy(CloseableHttpResponse closeableHttpResponse) throws IOException {
        ...
        if (closeableHttpResponse.getStatusLine().getStatusCode() == 200) {
            ...
            List<ClientCertificateDTO> clientCertificateDataDTOList;
            Type listType = new TypeToken<List<ClientCertificateDTO>>() {

            }.getType();
            clientCertificateDataDTOList = new Gson().fromJson(content, listType);

            try {
                PrivilegedCarbonContext.startTenantFlow();
                PrivilegedCarbonContext.getThreadLocalCarbonContext().setTenantDomain(tenantDomain, true);
                tenantFlowStarted = true;
                for (ClientCertificateDTO clientCertificatedataDTO : clientCertificateDataDTOList) {
                    CertificateManagerImpl.getInstance()
                            .addApplicationClientCertificateToGateway(clientCertificatedataDTO.getCertificate(),
                                    clientCertificatedataDTO.getUUID());
                }
            } finally {
    ...
    private CloseableHttpResponse invokeService(String endpoint, String tenantDomain)
        ...
        byte[] credentials = Base64.encodeBase64(
                (username + APIConstants.DELEM_COLON + password).getBytes(APIConstants.DigestAuthConstants.CHARSET));
        int port = url.getPort();
        String protocol = url.getProtocol();
        method.setHeader(APIConstants.AUTHORIZATION_HEADER_DEFAULT,
                APIConstants.AUTHORIZATION_BASIC + new String(credentials, APIConstants.DigestAuthConstants.CHARSET));
