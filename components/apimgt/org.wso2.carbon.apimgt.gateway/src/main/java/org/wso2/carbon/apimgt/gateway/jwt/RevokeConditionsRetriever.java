...
import org.wso2.carbon.apimgt.gateway.dto.RevokeConditionsDTO;
 ...
 */
public class RevokeConditionsRetriever extends TimerTask {

    private static final Log log = LogFactory.getLog(RevokeConditionsRetriever.class);

    @Override
    public void run() {

        log.debug("Starting web service based revoke condition retrieving process.");
        loadRevokeConditionsFromWebService();
    }

    /**
     * This method will retrieve revoke conditions by calling a web service.
     * Used to retrieve revoked JWT tokens, revoked consumer keys and revoked users.
     *
     * @return {@link RevokeConditionsDTO}.
     */
    private RevokeConditionsDTO retrieveRevokeConditionsData() {
        ...
        try {
            ...
            try (CloseableHttpResponse httpResponse = APIUtil.executeHTTPRequestWithRetries(method, httpClient)) {
                ...
                throw new DataLoadingException("Error while retrieving revoke conditions for JWT tokens", e);
            }

            if (responseString != null && !responseString.isEmpty()) {
                return new Gson().fromJson(responseString, RevokeConditionsDTO.class);
            }
        } catch (IOException | DataLoadingException e) {
            log.error("Exception when retrieving revoke conditions for JWT tokens from remote endpoint ", e);
        }
        return null;
    }

    private void loadRevokeConditionsFromWebService() {

        RevokeConditionsDTO revokeConditionsDTO = retrieveRevokeConditionsData();
    ...
    private void loadRevokedJWTTokens(RevokeConditionsDTO revokeConditionsDTO) {
        if (revokeConditionsDTO.getRevokedJWTList() != null && !revokeConditionsDTO.getRevokedJWTList().isEmpty()) {
            for (RevokedJWTTokenDTO revokedJWTToken : revokeConditionsDTO.getRevokedJWTList()) {
    ...
    private void loadRevokedConsumerKeys(RevokeConditionsDTO revokeConditionsDTO) {
        if (revokeConditionsDTO.getRevokedConsumerKeyList() != null && !revokeConditionsDTO.getRevokedConsumerKeyList()
                .isEmpty()) {
            for (RevokedJWTConsumerKeyDTO revokedConKeyDTO : revokeConditionsDTO.getRevokedConsumerKeyList()) {
                InternalRevokedJWTDataHolder.getInstance().addInternalRevokedJWTClientIDToMap(
    ...
    private void loadRevokedUsers(RevokeConditionsDTO revokeConditionsDTO) {
        if (revokeConditionsDTO.getRevokedUserList() != null && !revokeConditionsDTO.getRevokedUserList().isEmpty()) {
            for (RevokedJWTUserDTO revokedJWTUserDTO : revokeConditionsDTO.getRevokedUserList()) {
                if ("USER_ID".equals(revokedJWTUserDTO.getSubjectIdType())) {
                    InternalRevokedJWTDataHolder.getInstance().addInternalRevokedJWTUserIDToMap(
                            revokedJWTUserDTO.getSubjectId(), revokedJWTUserDTO.getRevocationTime());
                    if (log.isDebugEnabled()) {
                        log.debug("User ID : " + revokedJWTUserDTO.getSubjectId() + " added to the revoke map.");
                    }
                } else if ("CLIENT_ID".equals(revokedJWTUserDTO.getSubjectIdType())) {
                    InternalRevokedJWTDataHolder.getInstance().addInternalRevokedJWTClientIDToAppOnlyMap(
                            revokedJWTUserDTO.getSubjectId(), revokedJWTUserDTO.getRevocationTime());
                    if (log.isDebugEnabled()) {
                        log.debug("Client ID : " + revokedJWTUserDTO.getSubjectId() + " added to the revoke map.");
                    }
                }
            }
        } else {
     ...
     */
    public void startRevokeConditionsRetriever() {
