            ...
            if (scopesValid) {
                if (APISecurityUtils.getAuthenticationContext(synCtx) == null) {
                    ...
                    authContext.setStopOnQuotaReach(
                            true);//Since we don't have details on unauthenticated tier we setting stop on quota
                    // reach true
                    ...
                    authContext.setApplicationId(domainQualifiedUserName); //Set username as application ID in basic
                    // auth scenario
                    authContext.setApplicationUUID(domainQualifiedUserName); //Set username as application ID in
                    // basic auth scenario
                    authContext.setSubscriber(APIConstants.BASIC_AUTH_APPLICATION_OWNER); //Set application owner in
                    // basic auth scenario
    ...
    private String[] extractBasicAuthCredentials(String basicAuthHeader) throws APISecurityException {
        ...
        if (basicAuthHeader == null) {
            ...
            if (basicAuthHeader.contains(basicAuthKeyHeaderSegment)) {
                String basicAuthKey = new String(Base64.decodeBase64(
