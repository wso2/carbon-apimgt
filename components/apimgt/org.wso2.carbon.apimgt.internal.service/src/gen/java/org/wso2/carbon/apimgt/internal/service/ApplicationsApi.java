...
ApplicationsApiService delegate = new ApplicationsApiServiceImpl();
    ...
    public Response applicationsGet(@ApiParam(value = "This is used to specify the tenant domain, where the resource need to be   retrieved from. " ,required=true)@HeaderParam("xWSO2Tenant") String xWSO2Tenant,  @ApiParam(value = "**Search condition**.  Application ID of the application ")  @QueryParam("appId") Integer appId,  @ApiParam(value = "Whether to include system org API artifacts ")  @QueryParam("includeSystemOrgArtifacts") Boolean includeSystemOrgArtifacts) throws APIManagementException{
        return delegate.applicationsGet(xWSO2Tenant, appId, includeSystemOrgArtifacts, securityContext);
    }
}
