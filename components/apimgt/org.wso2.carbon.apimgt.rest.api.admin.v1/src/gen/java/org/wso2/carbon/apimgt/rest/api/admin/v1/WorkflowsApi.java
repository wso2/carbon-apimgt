...
WorkflowsApiService delegate = new WorkflowsApiServiceImpl();
    ...
    @ApiResponses(value = { 
        ...
        @ApiResponse(code = 404, message = "Not Found. The specified resource does not exist.", response = ErrorDTO.class),
        @ApiResponse(code = 406, message = "Not Acceptable. The requested media type is not supported.", response = ErrorDTO.class) })
    public Response getWorkflowByExternalRef(@ApiParam(value = "from the externel workflow reference we decide what is the the pending request that the are requesting. ",required=true) @PathParam("externalWorkflowRef") String externalWorkflowRef) throws APIManagementException{
        return delegate.getWorkflowByExternalRef(externalWorkflowRef, securityContext);
    }
    ...
    @ApiResponses(value = { 
        ...
        @ApiResponse(code = 400, message = "Bad Request. Invalid request or validation error.", response = ErrorDTO.class),
        @ApiResponse(code = 404, message = "Not Found. The specified resource does not exist.", response = ErrorDTO.class) })
    public Response updateWorkflowStatus( @NotNull @ApiParam(value = "Workflow reference id ",required=true)  @QueryParam("workflowReferenceId") String workflowReferenceId, @ApiParam(value = "Workflow event that need to be updated " ,required=true) WorkflowDTO workflowDTO) throws APIManagementException{
        return delegate.updateWorkflowStatus(workflowReferenceId, workflowDTO, securityContext);
    }
}
