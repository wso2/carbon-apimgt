...
ApplicationsApiService delegate = new ApplicationsApiServiceImpl();
    ...
    @ApiOperation(value = "Change Application Owner", notes = "**Deprecated.** This API will be removed in a future release. Use `/applications/{applicationId}/change-settings` instead.  This operation is used to change the owner of an Application. In order to change the owner of an application, we need to pass the new application owner as a query parameter ", response = Void.class, authorizations = {
    ...
    @POST
    @Path("/{applicationId}/change-settings")
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Application updated successfully", response = ApplicationDTO.class),
        @ApiResponse(code = 400, message = "Bad Request. Invalid request or validation error.", response = ErrorDTO.class),
        @ApiResponse(code = 404, message = "Not Found. The specified resource does not exist.", response = ErrorDTO.class) })
    public Response updateApplicationSettings(@ApiParam(value = "Application UUID ",required=true) @PathParam("applicationId") String applicationId, @ApiParam(value = "" ,required=true) ApplicationUpdateRequestDTO applicationUpdateRequestDTO) throws APIManagementException{
        return delegate.updateApplicationSettings(applicationId, applicationUpdateRequestDTO, securityContext);
    }
}
