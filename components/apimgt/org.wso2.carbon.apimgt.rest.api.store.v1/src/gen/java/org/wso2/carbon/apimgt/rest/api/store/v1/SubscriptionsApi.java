...
SubscriptionsApiService delegate = new SubscriptionsApiServiceImpl();
    ...
    public Response subscriptionsGet( @ApiParam(value = "**API ID** consisting of the **UUID** of the API. ")  @QueryParam("apiId") String apiId,  @ApiParam(value = "**Application Identifier** consisting of the UUID of the Application. ")  @QueryParam("applicationId") String applicationId,  @ApiParam(value = "How to combine **apiId** and **applicationId** when both are provided.  - `ALL`: both must match, returning only the subscription that links the given   API to the given application, or an empty list. - `ANY`, or when not provided: **apiId** takes precedence and **applicationId** is ignored. ", allowableValues="ANY, ALL")  @QueryParam("match") String match,  @ApiParam(value = "Application Group Id ")  @QueryParam("groupId") String groupId,  @ApiParam(value = "For cross-tenant invocations, this is used to specify the tenant domain, where the resource need to be   retrieved from. " )@HeaderParam("X-WSO2-Tenant") String xWSO2Tenant,  @ApiParam(value = "Starting point within the complete list of items qualified. ", defaultValue="0") @DefaultValue("0") @QueryParam("offset") Integer offset,  @ApiParam(value = "Maximum size of resource array to return. ", defaultValue="25") @DefaultValue("25") @QueryParam("limit") Integer limit,  @ApiParam(value = "Validator for conditional requests; based on the ETag of the formerly retrieved variant of the resourec. " )@HeaderParam("If-None-Match") String ifNoneMatch) throws APIManagementException{
        return delegate.subscriptionsGet(apiId, applicationId, match, groupId, xWSO2Tenant, offset, limit, ifNoneMatch, securityContext);
    }
