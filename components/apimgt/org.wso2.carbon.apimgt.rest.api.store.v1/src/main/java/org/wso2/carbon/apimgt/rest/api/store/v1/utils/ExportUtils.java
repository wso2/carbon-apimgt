...
public class ExportUtils {
    ...
    public static File exportApplication(Application exportApplication, APIConsumer apiConsumer,
        ...
        try {
            ...
            if (exploded) {
                // Directory ownership is handed off to the caller here - it must not be deleted on this path.
        ...
        } catch (IOException | APIImportExportException | APIManagementException e) {
            FileUtils.deleteQuietly(new File(exportApplicationBasePath));
    ...
    public static File exportApplications(List<Application> applications, APIConsumer apiConsumer,
            ExportFormat exportFormat, Boolean withKeys) throws APIManagementException {
        File tempDirectoryFolder = null;
        try {
            tempDirectoryFolder = CommonUtil.createTempDirectoryFromName(
                ...
                try {
                    ...
                    if (!copied) {
                        throw new APIImportExportException(
                                "No exported content found for application: " + application.getName());
                    }
                } catch (IOException e) {
                    throw new APIImportExportException(
