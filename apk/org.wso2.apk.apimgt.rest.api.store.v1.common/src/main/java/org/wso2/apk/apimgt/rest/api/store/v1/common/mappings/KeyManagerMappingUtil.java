...
import org.wso2.apk.apimgt.rest.api.store.v1.dto.KeyManagerInfoDTO;
...
/**
 * Utility class for KeyManager related operations
 */
public class KeyManagerMappingUtil {
    ...
    public static KeyManagerInfoDTO fromKeyManagerConfigurationDtoToKeyManagerInfoDto(
        ...
        keyManagerInfoDTO.setTokenType(
                KeyManagerInfoDTO.TokenTypeEnum.valueOf(keyManagerConfigurationDTO.getTokenType()));
        ...
        if (jsonObject.has(APIConstants.KeyManager.DISPLAY_TOKEN_ENDPOINT) && !jsonObject.get(
                APIConstants.KeyManager.DISPLAY_TOKEN_ENDPOINT).isJsonNull() && !jsonObject.get(
                APIConstants.KeyManager.DISPLAY_TOKEN_ENDPOINT).getAsString().trim().isEmpty()) {
        ...
        if (jsonObject.has(APIConstants.KeyManager.DISPLAY_REVOKE_ENDPOINT) && !jsonObject.get(
                APIConstants.KeyManager.DISPLAY_REVOKE_ENDPOINT).isJsonNull() && !jsonObject.get(
                APIConstants.KeyManager.DISPLAY_REVOKE_ENDPOINT).getAsString().trim().isEmpty()) {
        ...
        keyManagerInfoDTO.setApplicationConfiguration(
                fromKeyManagerConfigurationDto(keyManagerConfigurationDTO.getType()));
