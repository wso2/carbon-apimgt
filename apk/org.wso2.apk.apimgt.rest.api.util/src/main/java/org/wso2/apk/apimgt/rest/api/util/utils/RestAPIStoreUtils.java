...
public class RestAPIStoreUtils {
     ...
     */
    public static boolean isUserAccessAllowedForApplication(Application application) throws APIManagementException {
        String groupId;
    ...
    public static boolean isUserOwnerOfApplication(Application application) {
        ...
        if (application.getSubscriber().getName().equals(username)) {
            ...
            ConfigurationHolder configuration = ServiceReferenceHolder.getInstance()
                    .getAPIManagerConfigurationService().getAPIManagerConfiguration();
            String comparisonConfig = configuration.getFirstProperty(APIConstants.API_STORE_FORCE_CI_COMPARISIONS);
            return (StringUtils.isNotEmpty(comparisonConfig) && Boolean.parseBoolean(comparisonConfig));
        }
