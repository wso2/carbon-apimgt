 ...
 * under the License.
 */
package org.wso2.apk.apimgt.impl.wsdl;
...
import org.wso2.apk.apimgt.impl.utils.APIFileUtil;
import org.wso2.apk.apimgt.impl.utils.APIMWSDLReader;
import org.wso2.apk.apimgt.impl.wsdl.exceptions.APIMgtWSDLException;
import org.wso2.apk.apimgt.impl.wsdl.model.WSDLInfo;
import org.wso2.apk.apimgt.impl.wsdl.model.WSDLParamDefinition;
import org.wso2.apk.apimgt.impl.wsdl.model.WSDLSOAPOperation;
...
public class WSDL11SOAPOperationExtractor extends WSDL11ProcessorImpl {
                                            ...
                                            for (Object aSchema : typeList) {
                                                if (schemaUrl.equalsIgnoreCase(
                                                        ((Schema) aSchema).getElement()
                                                                .getAttribute(SOAPToRESTConstants.TARGET_NAMESPACE_ATTRIBUTE))) {
                                                    isInlineSchema = true;
                                                    break;
                                                }
                                            }
    ...
    private Property generateSwaggerModelForComplexType(Node current, ModelImpl model, Property currentProp,
        ...
        } else if (SOAPToRESTConstants.EXTENSION_NODE_NAME.equals(current.getLocalName())) {
            readExtensionModel(model, current);
        } else if (SOAPToRESTConstants.ATTRIBUTE_NODE_NAME.equals(current.getLocalName())) {
    ...
    private void readExtensionModel(ModelImpl model, Node node) throws APIManagementException {
        Node baseNode = node.getAttributes().getNamedItem(SOAPToRESTConstants.BASE_ATTR);
        if (baseNode == null) {
            return;
        }
        ...
        model.addProperty(SOAPToRESTConstants.BASE_CONTENT_KEYWORD, new RefProperty(refName));
        if (nsName == null) {
            return;
        }
    ...
    private void addAttributesToModel(ModelImpl model, NamedNodeMap namedNodeMap) {
        if (namedNodeMap == null || namedNodeMap.getNamedItem(SOAPToRESTConstants.NAME_ATTRIBUTE) == null) {
            return;
        }
        String name = namedNodeMap.getNamedItem(SOAPToRESTConstants.NAME_ATTRIBUTE).getNodeValue();
        Map<String, Property> properties = new HashMap<>();
        if (model.getProperties() == null || !model.getProperties().containsKey(SOAPToRESTConstants
                .ATTR_CONTENT_KEYWORD)) {
            ObjectProperty objectProperty = new ObjectProperty();
            objectProperty.setProperties(properties);
            model.addProperty(SOAPToRESTConstants.ATTR_CONTENT_KEYWORD, objectProperty);
        }
        ObjectProperty objectProperty = (ObjectProperty) model.getProperties().get(SOAPToRESTConstants.
                ATTR_CONTENT_KEYWORD);
        ...
        if (prevNode.hasChildNodes()) {
            ...
            for (int i = 0; i < childNodes.getLength(); i++) {
                if (childNodes.item(i).getAttributes() != null
                        && childNodes.item(i).getAttributes().getNamedItem(SOAPToRESTConstants.NAME_ATTRIBUTE) != null
                        && current
                        .getAttributes().getNamedItem(SOAPToRESTConstants.NAME_ATTRIBUTE).getNodeValue()
                        .equals(childNodes.item(i).getAttributes().getNamedItem(SOAPToRESTConstants.NAME_ATTRIBUTE)
                                .getNodeValue())) {
                    return true;
                }
            }
